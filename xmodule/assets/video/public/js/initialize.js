...
console.log("Within Initialize.js!");
/**
     * @function
     *
     ...
     *     All other modules, their parameters, public letiables, etc. are
...
function _renderElements(state) {
    ...
    if (state.videoType === 'youtube') {
        ...
        if (window.YT) {
            ...
            // There is only one global letiable window.onYouTubeIframeAPIReady which
            ...
            if (!_youtubeApiDeferred) {
                ...
                // initialization of the video module. However, since then the global letiable
...
function initialize(element) {
    ...
    // are "read only", so don't modify them. All letiable content lives in
    // 'state' object.
...
function trigger(objChain) {
    ...
    // At the end of the loop the letiable 'tmpObj' will either be the
