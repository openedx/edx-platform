...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # setup a request cache so we don't throttle the DB with all the
        # metadata inheritance requests
        ...
        # we've had a bug where the xml_attributes field can we rewritten as a
        ...
        # check for dangling discussion items, this can cause errors in the
        # forums
        def _get_discussion_items(module):
            ...
            for child in module.get_children():
                discussion_items = discussion_items + \
                    _get_discussion_items(child)

            return discussion_items

        discussion_items = _get_discussion_items(course)

        # now query all discussion items via get_items() and compare with the
        # tree-traversal
        queried_discussion_items = store.get_items(
            ['i4x', course.location.org, course.location.course,
