            ...
            logger.check(
                (
                    ...
                    (u"[video thumbnails] [run=1] [video-thumbnails-scraping-failed-with-unknown-exc] "
                     "[edx_video_id=super-soaker] [youtube_id=OscRe3pSP80] [course={}]".format(self.course.id))  # pylint: disable=unicode-format-string
                ),
                (
                    tasks_logger, 'ERROR',
                    (u"[video thumbnails] [run=1] [video-thumbnails-scraping-failed-with-unknown-exc] "
                     "[edx_video_id=medium-soaker] [youtube_id=OscRe3pSP81] [course={}]".format(self.course_2.id))  # pylint: disable=unicode-format-string
                )
            )
