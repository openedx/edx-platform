...
class CourseToLibraryTestCase(ContentLibrariesRestApiTest, ModuleStoreTestCase):
    ...
    def test_problem_sync(self):
        ...
        """
        # First, create the problem in the course, using the upstream problem as a template:
        ...
        downstream_olx = self._get_course_block_olx(downstream_problem1["locator"])
        # Notice that per UpstreamSyncMixin.get_customizable_fields(), some fields like
        # weight and max_attempts are DROPPED entirely from the upstream version when creating the downstream:
        self.assertXmlEqual(downstream_olx, f"""
            <problem
