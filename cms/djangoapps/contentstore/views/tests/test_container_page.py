    ...
    def test_container_html(self):
        self._test_html_content(
            ...
            expected_section_tag=(
                ...
                'data-locator="{0}" data-course-key="{0.course_key}">'.format(self.child_container.location)  # pylint: disable=unicode-format-string,line-too-long
            ),
    ...
    def test_container_on_container_html(self):
        ...
        def test_container_html(xblock):
            self._test_html_content(
                ...
                expected_section_tag=(
                    u'<section class="wrapper-xblock level-page is-hidden studio-xblock-wrapper" '
                    'data-locator="{0}" data-course-key="{0.course_key}">'.format(draft_container.location)  # pylint: disable=unicode-format-string,line-too-long
                ),
                expected_breadcrumbs=(
                    ur'<a href="/course/{course}{section_parameters}" class="{classes}">\s*Week 1\s*</a>\s*'
                    ur'<a href="/course/{course}{subsection_parameters}" class="{classes}">\s*Lesson 1\s*</a>\s*'
