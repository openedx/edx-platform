...
class VideoUploadPostTestsMixin:
    ...
    @patch('cms.djangoapps.contentstore.video_storage_handlers.storage_service_bucket')
    def test_post_success(self, mock_storage_service_bucket):
        files = [
            {
                'file_name': 'first.mp4',
        ...
        # Mock bucket and S3 client
        mock_bucket = Mock()
        mock_bucket.name = 'test-bucket'
        mock_s3_client = Mock()
        mock_bucket.meta.client = mock_s3_client
        
        # Configure the mock to return presigned URLs
        def generate_presigned_url_side_effect(operation, Params, ExpiresIn):
            return f'http://example.com/url_{Params["Key"]}'
        
        mock_s3_client.generate_presigned_url.side_effect = generate_presigned_url_side_effect
        mock_storage_service_bucket.return_value = mock_bucket
        ...
        for i, file_info in enumerate(files):
            ...
            self.assertIn('client_video_id', metadata)
            self.assertEqual(metadata['client_video_id'], file_info['file_name'])
            self.assertIn('course_key', metadata)
            ...
            self.assertIn('http://example.com/url_', response_file['upload_url'])
...
class VideosHandlerTestCase(
    ...
    @ddt.unpack
    def test_video_supported_file_formats(self, files, expected_status, mock_storage_service_bucket):
        """
    ...
    @override_settings(AWS_ACCESS_KEY_ID='test_key_id', AWS_SECRET_ACCESS_KEY='test_secret')
    @patch('cms.djangoapps.contentstore.video_storage_handlers.storage_service_bucket')
    def test_upload_with_non_ascii_charaters(self, mock_storage_service_bucket):
        """
        Test that video uploads throws error message when file name contains special characters.
        """
        # Mock bucket (though it shouldn't be called due to early validation)
        mock_bucket = Mock()
        mock_storage_service_bucket.return_value = mock_bucket
    ...
    def test_devstack_upload_connection(self, mock_storage_service_bucket):
    ...
    @patch('cms.djangoapps.contentstore.video_storage_handlers.storage_service_bucket')
    def test_send_course_to_vem_pipeline(self, mock_storage_service_bucket):
        """
        Test that uploads always go to VEM S3 bucket by default.
        """
        # Mock bucket and S3 client
        mock_bucket = Mock()
        mock_bucket.name = settings.VIDEO_UPLOAD_PIPELINE['VEM_S3_BUCKET']
        mock_s3_client = Mock()
        mock_bucket.meta.client = mock_s3_client
        ...
        self.assertEqual(response.status_code, 200)
        mock_storage_service_bucket.assert_called_once()
        # Verify bucket name is the VEM bucket
        self.assertEqual(mock_bucket.name, settings.VIDEO_UPLOAD_PIPELINE['VEM_S3_BUCKET'])
    ...
    def test_video_upload_token_in_meta(self, data, mock_storage_service_bucket):
        """
        ...
        # Track the metadata that gets passed to generate_presigned_url
        captured_metadata = {}
        def generate_presigned_url_side_effect(operation, Params, ExpiresIn):
            captured_metadata.update(Params.get('Metadata', {}))
            return f'http://example.com/url_{Params["Key"]}'
        ...
        with patch.object(WaffleFlagCourseOverrideModel, 'override_value', return_value=data['course_override']):
            with override_waffle_flag(DEPRECATE_YOUTUBE, active=data['global_waffle']):
                ...
                # Check if token is in metadata based on expectation
                if data['expect_token']:
                    self.assertIn('course_video_upload_token', captured_metadata)
                    self.assertEqual(captured_metadata['course_video_upload_token'], self.test_token)
                else:
                    self.assertNotIn('course_video_upload_token', captured_metadata)
...
class TranscriptPreferencesTestCase(VideoUploadTestBase, CourseTestCase):
    ...
    def test_transcript_preferences_metadata(self, transcript_preferences, is_video_transcript_enabled,
                                             mock_transcript_preferences, mock_storage_service_bucket):
        """
        ...
        if is_video_transcript_enabled and transcript_preferences:
            self.assertIn('transcript_preferences', captured_metadata)
            self.assertEqual(captured_metadata['transcript_preferences'], json.dumps(transcript_preferences))
        else:
            self.assertNotIn('transcript_preferences', captured_metadata)
...
class GetStorageBucketTestCase(TestCase):
    ...
    def test_storage_bucket(self):
        ...
        # Use boto3's generate_presigned_url method on the bucket's client
        upload_url = bucket.meta.client.generate_presigned_url(
            'put_object',
            Params={
                'Bucket': bucket.name,
                'Key': key_name,
