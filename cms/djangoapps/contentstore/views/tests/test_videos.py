...
class VideoUploadTestCase(CourseTestCase):
    ...
    def setup_and_post_video_uploads(self, s3_key=None, s3_connection=None):  # pylint: disable=unused-argument
        ...
        self.files = [
            {'file_name': 'file1', 'content_type': 'video/mp4'},
            {'file_name': 'file2', 'content_type': 'video/webm'},
        ]
    ...
    def find_in_list(self, value, list_of_dicts, key):
        """
    ...
    def test_video_index(self):
        ...
        for expected_file in self.files:
            ...
            video = self.find_in_list(file_name, videos, 'client_video_id')

            # ensure video is in VAL
            self.assertIsNotNone(get_video_info(video['edx_video_id']))

            # status
            self.assertEquals(video['status'], 'upload')

            # upload date
            upload_date = dateutil.parser.parse(video['created'])
