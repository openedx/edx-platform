...
class ClipboardPasteTestCase(ModuleStoreTestCase):
    ...
    def test_copy_and_paste_video(self):
        ...
        self.assertEqual(len(orig_vertical.children), 4)
        ...
        self.assertEqual(len(updated_vertical.children), 5)
        self.assertEqual(updated_vertical.children[-1], new_block_key)
        # And it should match the original:
        orig_video = modulestore().get_item(video_key)
        new_video = modulestore().get_item(new_block_key)
        self.assertEqual(new_video.youtube_id_1_0, orig_video.youtube_id_1_0)
        # The new block should store a reference to where it was copied from
        self.assertEqual(new_video.copied_from_block, video_key)
