...
def _delete_entrance_exam(request, course_key):
    ...
    course_children = modulestore().get_items(
        course_key,
        qualifiers={'metadata.in_entrance_exam': True}
    )
    for course_child in course_children:
        delete_item(request, course_child.location)
        milestones_api.remove_content_references(unicode(course_child.location))
