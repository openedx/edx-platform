...
__all__ = [
    ...
    'transcript_ai_generated_handler'
]
...
@login_required
@require_GET
def transcript_ai_generated_handler(request, course_key_string):
    """
    JSON view handler to create ai generated transcripta.
    ...
    print('finished buuilding')
    for video in videos:
        video_file = ''
        for encoded_video in video["encoded_videos"]:
            current_video_file = encoded_video["url"]
            if video_file == '' and current_video_file.endswith('.mp4'):
                video_file = current_video_file
                # edx_video_id = video["edx_video_id"]
                print(video_file)
                result = model.transcribe(video_file, fp16=False, verbose=True)
                text = result["text"]
                print(text)
                # Save the text to a new file in the same directory
                srtFilename = ('ai_generated_en.srt')
                srt = ''
                ...
                print(srt)
            # upload_transcripts({'edx_video_id': edx_video_id, srtFilename})
    return JsonResponse()
...
def transcript_preferences_handler(request, course_key_string):
    ...
    if request.method == 'POST':
        ...
        # add a check if the provider is autogenerated, if yes call create_ai_generated_transcripts
        if error:
...
def videos_index_html(course, pagination_conf=None):
    ...
    context = {
        ...
        'autogenerated_transcripts': reverse_course_url('transcript_ai_generated_handler', str(course.id)),
