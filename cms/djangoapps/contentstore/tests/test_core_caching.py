...
# Tests for https://edx.lighthouseapp.com/projects/102637/tickets/112
# -updating-asset-does-not-refresh-the-cached-copy
    unicodeLocation = Location(
        u'c4x', u'mitX', u'800', u'thumbnail', u'monsters.jpg')
    # Note that some of the parts are strings instead of unicode strings
    nonUnicodeLocation = Location(
        'c4x', u'mitX', u'800', 'thumbnail', 'monsters.jpg')
    ...
    def test_put_and_get(self):
        ...
        self.assertEqual(
            self.mockAsset.content, get_cached_content(
                self.unicodeLocation).content,
            'should be stored in cache with unicodeLocation')
        self.assertEqual(
            self.mockAsset.content, get_cached_content(
                self.nonUnicodeLocation).content,
