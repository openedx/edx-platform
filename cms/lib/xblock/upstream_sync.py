...
def sync_from_upstream(downstream: XBlock, user: AbstractUser) -> None:
    ...
    """
    if not (link_and_upstream := _load_upstream_link_and_block(downstream, user)):
        return
    link, upstream = link_and_upstream
...
def fetch_customizable_fields(*, downstream: XBlock, user: AbstractUser, upstream: XBlock | None = None) -> None:
    """
    ...
    Otherwise, load the block specified by  `downstream.upstream`, which may raise BadUpstream/BadDownstream.
    """
    if not upstream:
        if not (link_and_upstream := _load_upstream_link_and_block(downstream, user)):
            return  # `downstream.upstream is None` -> nothing to do
        _link, upstream = link_and_upstream
    _update_customizable_fields(upstream=upstream, downstream=downstream, only_fetch=True)


def _load_upstream_link_and_block(downstream: XBlock, user: AbstractUser) -> tuple[UpstreamLink, XBlock] | None:
    """
    Load the upstream metadata and content for a downstream block (or None if it is not linked).

    Can raise BadUpstream and BadDownstream.
...
def _update_customizable_fields(*, upstream: XBlock, downstream: XBlock, only_fetch: bool) -> None:
    """
    For each customizable field `downstream.blah`:

    1. Fetch `upstream.blah` and save it in `downstream.upstream_blah`.
    2. If not only_fetch, and if `downstream.blah` isn't customized, update it from `upstream.blah`.
    """
