 ...
 */
 define([
    ...
    "js/views/utils/create_utils_base",
  ], function (
    $,
    Backbone,
    _,
    ...
    SettingsXBlockModal = CourseOutlineXBlockModal.extend({
      getTitle: function () {
        return StringUtils.interpolate(gettext("{display_name} Settings"), {
    ...
    UploadXBlockModal = CourseOutlineXBlockModal.extend({
      getTitle: function () {
        return StringUtils.interpolate(gettext("{display_name} File Upload"), {
          display_name: this.model.get("display_name"),
        });
      },
      ...
      initializeEditors: function () {
        ...
        // uploadButton.setAttribute("form", "fileUploadForm")
        // uploadButton.innerText = "UPLOAD"
  
        var chapter_list = function (chapter_name) {
          const url_split = url.split("/");
          var course = url_split[url_split.length - 1];
          course = course.split("#");
        ...
        chapter_list(this.model.get("display_name"));
  
        modal_content.appendChild(uploadBox);
  
        uploadButton.onclick = () => {
          let button = document.querySelector("#uploadChapterFile");
          button.click();
        };
  
        const myForm = document.querySelector("#fileUploadForm");
        ...
        myForm.addEventListener("submit", (e) => {
          ...
          var course = url_split[url_split.length - 1];
          course = course.split("#");
          const document_name = myFile.files[0].name.split(".")[0];
          const document_type = myFile.files[0].name.split(".")[1];
  
          const formData = new FormData();
          const chapter_name = this.model.get("display_name");
          //   var options = {
          //         'Content-Type': 'multipart/form-data'
          // };
          // formData.append("header",options);
  
          formData.append("document", myFile.files[0]);
          formData.append("course", course[0]);
          formData.append("chapter_name", chapter_name);
          formData.append("document_name", document_name);
          formData.append("document_type", document_type);
          $.postFormData("../upload_doc", formData, function (data) {alert("Document Uploaded Successfully")});
        });
  
        // myForm.addEventListener('submit', e =>{
        //     e.preventDefault();
        //     const endpoint = "http://www.example.net";
        //     const formData = new FormData();
      ...
      // events: _.extend({}, CourseOutlineXBlockModal.prototype.events, {
      //     'click .action-upload-file': 'uploadMyFile',
      // }),
      // uploadMyFile: function(){
      //     console.log("Inside uploadMyFile 2")
      // },
      addActionButtons: function () {
        this.addActionButton("upload-file", gettext("Upload"), true);
    ...
    PublishXBlockModal = CourseOutlineXBlockModal.extend({
      ...
      getTitle: function () {
        return StringUtils.interpolate(gettext("Publish {display_name}"), {
          display_name: this.model.get("display_name"),
        });
      },
  
      getIntroductionMessage: function () {
        return StringUtils.interpolate(gettext("Publish all unpublished changes for this {item}?"), {
    ...
    HighlightsXBlockModal = CourseOutlineXBlockModal.extend({
      ...
      getTitle: function () {
        return StringUtils.interpolate(gettext("Highlights for {display_name}"), {
          display_name: this.model.get("display_name"),
        });
      },
        ...
        var html = this.template(
          $.extend(
            {},
            {
              ...
              isTimedExam: isTimeLimited && !(isProctoredExam || isPracticeExam || isOnboardingExam),
    ...
    BaseDateEditor = AbstractEditor.extend({
      ...
      afterRender: function () {
        ...
        if (this.model.get(this.fieldName)) {
          DateUtils.setDate(this.$("input.date"), this.$("input.time"), this.model.get(this.fieldName));
        }
      },
    });
    ...
    SelfPacedDueDateEditor = AbstractEditor.extend({
      ...
      showProjectedDate: function () {
        ...
        this.$("#relative_weeks_due_start_date").text(
          startDateList[2] + " " + startDateList[1] + ", " + startDateList[3]
        );
        ...
        this.$("#relative_weeks_due_projected_due_in").text(
          projectedDateList[2] + " " + projectedDateList[1] + ", " + projectedDateList[3]
        );
    ...
    TimedExaminationPreferenceEditor = AbstractEditor.extend({
      ...
      afterRender: function () {
        ...
        this.setExamType(
          this.model.get("is_time_limited"),
          this.model.get("is_proctored_exam"),
          this.model.get("is_practice_exam"),
          this.model.get("is_onboarding_exam")
        );
    ...
    GradingEditor = AbstractEditor.extend({
      ...
      handleGradingSelect: function (event) {
        event.preventDefault();
        if (
          this.$("#grading_type").val() !== "notgraded" &&
          course.get("self_paced") &&
          course.get("is_custom_relative_dates_active")
        ) {
    ...
    UnitAccessEditor = AbstractVisibilityEditor.extend({
      ...
      afterRender: function () {
        ...
        if (this.model.attributes.group_access) {
          ...
          if (keys.length === 1) {
            // should be only one partition key
    ...
    ContentVisibilityEditor = AbstractVisibilityEditor.extend({
      ...
      getContext: function () {
        return $.extend({}, AbstractVisibilityEditor.prototype.getContext.call(this), {
    ...
    ShowCorrectnessEditor = AbstractEditor.extend({
      ...
      getContext: function () {
        return $.extend({}, AbstractEditor.prototype.getContext.call(this), {
          self_paced: course.get("self_paced") === true,
        });
      },
    });
    ...
    return {
      getModal: function (type, xblockInfo, options) {
        if (type === "edit") {
          ...
          return this.getUploadModal(xblockInfo, options);
      ...
      getEditModal: function (xblockInfo, options) {
        console.log("GetEditModal : ", xblockInfo);
        var tabs = [];
        var editors = [];
        var advancedTab = {
          name: "advanced",
        ...
        return new SettingsXBlockModal(
          $.extend(
            {
              tabs: tabs,
              editors: editors,
      ...
      getUploadModal: function (xblockInfo, options) {
        return new UploadXBlockModal(
          $.extend(
            {
              model: xblockInfo,
            },
      ...
      getHighlightsModal: function (xblockInfo, options) {
        return new HighlightsXBlockModal(
          $.extend(
            {
      ...
      getHighlightsEnableModal: function (xblockInfo, options) {
        return new HighlightsEnableXBlockModal(
          $.extend(
            {
