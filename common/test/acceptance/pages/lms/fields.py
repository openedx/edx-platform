...
class FieldsMixin(object):
    ...
    def value_for_dropdown_field(self, field_id, value=None):
        ...
        if self.mode_for_field(field_id) == 'edit':
            def get_text(query):
                return (True, get_selected_option_text(query))

            # Wrap the retrieval of the text in a Promise to prevent errors
            # from being raised while the DOM is still being rewritten
            return Promise(lambda: get_text(query), 'Retrieved selected text from field {}'.format(field_id))
        else:
