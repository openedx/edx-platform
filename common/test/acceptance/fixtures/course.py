...
class CourseFixture(XBlockContainerFixture):
    ...
    def studio_course_outline_as_json(self):
        ...
        try:
            ...
            raise FixtureError(
                "Could not decode course outline as JSON: '{}'".format(response)
            )
    ...
    def _create_course(self):
        ...
        if err is not None:
            raise FixtureError("Could not create course {}.  Error message: '{}'".format(self, err))
    ...
    def _configure_course(self):
        ...
        try:
            ...
            raise FixtureError(
                "Could not decode course details as JSON: '{}'".format(details)
            )
    ...
    def _install_course_handouts(self):
        ...
        if not response.ok:
            raise FixtureError(
                "Could not update course handouts with {}.  Status was {}".format(url, response.status_code))
