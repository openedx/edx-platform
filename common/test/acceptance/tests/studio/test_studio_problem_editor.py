...
class ProblemComponentEditor(ContainerBase):
    ...
    def test_user_can_view_metadata(self):
        ...
        """
        default_settings = [
            (u'Display Name', u'Blank Common Problem'),
            (u'Matlab API key', u''),
            (u'Maximum Attempts', u''),
            (u'Problem Weight', u''),
            (u'Randomization', u'Never'),
            (u'Show Answer', u'Finished'),
            (u'Show Reset Button', u'False'),
            (u'Timer Between Attempts', u'0')
        ]
    ...
    def test_user_can_set_html_in_display_name(self):
        ...
        self.problem_editor.save()
        component_name = self.unit.xblock_titles[0]
        self.assertEqual(component_name, '<script>alert("test")</script>')
    ...
    def test_user_cannot_type_decimal_values(self):
        ...
        self.assertEqual(field_value, '2.34')

    @skip('As per lettuce implementation, the valued should be +3 but we are getting -3 after the save.')
    ...
    def test_user_can_select_values(self):
        ...
        self.assertEqual(dropdown_value, 'Per Student')
        self.problem_editor.select_from_dropdown(dropdown_name, 'Never')
        dropdown_value = self.problem_editor.get_value_from_the_dropdown(dropdown_name)
        self.assertEqual(dropdown_value, 'Never')
