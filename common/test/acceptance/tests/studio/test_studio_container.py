...
class EditVisibilityModalTest(ContainerBase):
    ...
    def verify_component_validation_error(self, component):
        ...
        def check_func():
            """
            Check that the validation error is present and has expected text.
            """
            error_present = component.has_validation_error
            correct_label = (component.validation_error_text == self.VALIDATION_ERROR_LABEL)
            correct_msg = ([self.VALIDATION_ERROR_MESSAGE] == component.validation_error_messages)
            return error_present and correct_label and correct_msg
