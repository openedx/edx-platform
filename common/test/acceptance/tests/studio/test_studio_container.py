...
class EditVisibilityModalTest(ContainerBase):
    ...
    def verify_selected_labels(self, visibility_editor, expected_labels):
        ...
        self.assertItemsEqual(expected_labels, visibility_editor.selected_options)
    ...
    def verify_component_validation_error(self, component, expected_num_missing):
        """
        Verify that we see validation errors for the given component.
        """
        self.assertTrue(component.has_validation_error)
        self.assertEqual(component.validation_error_text, 'This component has validation issues.')
        expected_error_messages = list(u'This xblock refers to a deleted or invalid content group.') * expected_num_missing
        for expected_message, actual_message in zip(expected_error_messages, component.validation_error_messages):
    ...
    def verify_visibility_set(self, component, is_set):
        ...
        if is_set:
            self.assertIn(group_visibility_message, self.container_page.sidebar_visibility_message)
            self.assertTrue(component.has_group_visibility_set)
        else:
            self.assertNotIn(group_visibility_message, self.container_page.sidebar_visibility_message)
    ...
    def remove_missing_groups(self, visibility_editor):
        """
        Given a visibility editor with some groups selected, reset
        its visible to all students and staff.
        """
    ...
    def test_select_cats(self):
        ...
        Scenario: The component visibility modal can be set to be visible to one content group.
            ...
            And I select 'Cats'
            And I save the modal
            Then the visibility selection should be 'Cats' and 'Specific Content Groups'
            And the container page should display 'Some content in this unit is only visible to particular groups'
        """
        self.select_and_verify_saved(self.html_component, ['Cats'])
        self.verify_visibility_set(self.html_component, True)
    ...
    def test_missing_group(self):
        """
        Scenario: The component visibility modal shows a validation error when visibility is set to an unknown group id.
            Given I have a unit with one component
            And that component's group access specifies an invalid group id
            When I go to the container page for that unit
            Then I should see a validation error message on that unit's component
            And I open the visibility editor modal for that unit's component
            Then I should see that I have selected a deleted group
            And the container page should display 'Some content in this unit is only visible to particular groups'
            And I select 'All Students and Staff'
            And I save the modal
            Then the visibility selection should be 'All Students and Staff'
            And the container page should not display 'Some content in this unit is only visible to particular groups'
        """
        self.update_component(self.html_component, {'group_access': {0: [2]}})
        self.verify_component_validation_error(self.html_component, 1)
        visibility_editor = self.edit_component_visibility(self.html_component)
        self.verify_selected_labels(visibility_editor, [self.MISSING_GROUP_LABEL])
    ...
    def test_missing_groups(self):
        ...
        self.verify_component_validation_error(self.html_component, 2)
        visibility_editor = self.edit_component_visibility(self.html_component)
