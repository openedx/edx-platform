...
class CertificatesTest(StudioCourseTest):
    ...
    def test_can_create_and_delete_signatories_of_certficate(self):
        """
        Scenario: Ensure that certificate signatories can be created and deleted correctly.
        Given I have a course without certificates
        When I click button 'Add your first Certificate'
        And I set new the name, description and two signatories and click the button 'Create'
        Then I see the new certificate is added and has two signatories inside it
        When I click 'Edit' button of certificate panel
        And I click on 'Delete' icon in the top right corner of second signatory panel
        Then I see the signatory deleted from certificate and one signatory left
        And remaining signatory panel should not have delete icon
        """
        self.certificates_page.visit()
        certificate = self.create_and_verify_certificate(
            "New Certificate",
        ...
        self.assertEqual(len(self.certificates_page.certificates), 1)
        self.assertEqual(len(self.certificates_page.certificates[0].signatories), 2)
        # Delete the signatory in certificate
        certificate.edit()
        signatory = certificate.signatories[0]
        signatory.edit()
        signatory.delete()

        self.assertEqual(len(self.certificates_page.certificates[0].signatories), 1)

        # remaining signatory panel should not have delete icon
        signatory = certificate.signatories[0]
        self.assertFalse(signatory.delete_icon_is_present)
