...
def update_enrollment(user_id, course_id, mode=None, is_active=None, enrollment_attributes=None, include_expired=False):
    ...
    if enrollment is None:
        msg = "Course Enrollment not found for user {user} in course {course}".format(user=user_id, course=course_id)
        log.warn(msg)
...
def get_course_enrollment_details(course_id, include_expired=False):
    ...
    if cached_enrollment_data:
        log.info("Get enrollment data for course %s (cached)", course_id)
    ...
    try:
        ...
        raise errors.CourseEnrollmentError("An unexpected error occurred while retrieving course enrollment details.")

    log.info("Get enrollment data for course %s", course_id)
...
def _data_api():
    ...
    try:
        ...
        log.exception("Could not load module at '{path}'".format(path=api_path))
