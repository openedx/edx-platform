...
class SendReferralFollowUpEmails(TestCase):
    ...
    @mock.patch('philu_commands.management.commands.send_referral_follow_up_email.task_send_referral_follow_up_email')
    def test_send_referral_follow_up_command(self, mock_task_send_referral_follow_up_email):
        """
        This test will return a list of email address. The email address will follow certain criteria. We are getting
        emails from 'SmartReferral' tables so only those entries will be picked that are at-least three days old. If an
        email address is registered on our platform(philu) it will be ignored. Lastly if the 'is_referral_step_complete'
        flag is 'True' that email will also be ignored.
        """
        ...
        expected_call_arguments = [test_email4]
