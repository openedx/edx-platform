...
from student.tests.factories import AnonymousUserIdFactory, CourseEnrollmentFactory, UserFactory
...
def test_autoscore_ondemand_course_ora_successfully(mock_log_info, mock_find_and_autoscore_submissions, enrollments):
    """
    ...
    for enrollment in enrollments:
        ...
        if enrollment.course_overview.self_paced:
            expected_submission_uuids.insert(0, unicode(submission.uuid))
        if enrollment.is_active:
            expected_enrollments.append(enrollment)

    call_command('autoscore_ondemand_course_ora')

    if not expected_submission_uuids:
        assert mock_log_info.called
        assert not mock_find_and_autoscore_submissions.called
    else:
        assert not mock_log_info.called
        mock_find_and_autoscore_submissions.assert_called_once_with(expected_enrollments, expected_submission_uuids)


@pytest.mark.django_db
@mock.patch('philu_commands.management.commands.autoscore_ondemand_course_ora.find_and_autoscore_submissions')
@mock.patch('philu_commands.management.commands.autoscore_ondemand_course_ora.log.info')
def test_autoscore_ondemand_course_ora_no_ora_submission(mock_log_info, mock_find_and_autoscore_submissions):
    """
