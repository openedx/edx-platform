...
def set_logged_in_cookies(request, response, user):
    ...
    * JWT_AUTH_COOKIE: JSON Web Token with user information and can be used as an access token.
    ...
    if user.is_authenticated and not user.is_anonymous:
        ...
        _set_jwt_cookie(response, request, user)
...
def _set_jwt_cookie(response, request, user):
    """ Sets a cookie containing a JWT on the response. """
    cookie_settings = standard_cookie_settings(request)
    _set_jwt_expiration(cookie_settings)

    jwt = _build_jwt(user, cookie_settings['max_age'])
    response.set_cookie(
...
def _build_jwt(user, expires_in):
