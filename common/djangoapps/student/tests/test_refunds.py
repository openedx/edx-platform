...
class RefundableTest(SharedModuleStoreTestCase):
    ...
    def test_refundable(self, cutoff_date):
        ...
        cutoff_date.return_value = datetime.now(get_utc_timezone()) + timedelta(days=1)
    ...
    def test_refundable_expired_verification(self, cutoff_date):
        ...
        cutoff_date.return_value = datetime.now(get_utc_timezone()) + timedelta(days=1)
        self.verified_mode.expiration_datetime = datetime.now(get_utc_timezone()) - timedelta(days=1)
    ...
    def test_refundable_with_cutoff_date(self, cutoff_date):
        ...
        cutoff_date.return_value = datetime.now(get_utc_timezone()) - timedelta(minutes=5)
        assert not self.enrollment.refundable()
    ...
    def test_refund_cutoff_date(self, order_date_delta, course_start_delta, expected_date_delta, days):
        ...
        """
        now = datetime.now(get_utc_timezone()).replace(microsecond=0)
    ...
    @ddt.data(
        (datetime.now(get_utc_timezone()) + timedelta(days=1), True),
        (datetime.now(get_utc_timezone()) - timedelta(days=1), False),
        (datetime.now(get_utc_timezone()) - timedelta(minutes=5), False),
    )
    @ddt.unpack
    @httpretty.activate
    @override_settings(ECOMMERCE_API_URL=TEST_API_URL)
