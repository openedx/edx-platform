...
class CourseEnrollmentTests(SharedModuleStoreTestCase):  # lint-amnesty, pylint: disable=missing-class-docstring
    ...
    def test_upgrade_deadline(self):
        ...
        course_mode = CourseModeFactory(
            ...
            expiration_datetime=datetime.datetime.now(get_utc_timezone()) + datetime.timedelta(days=1)
        )
    ...
    def test_upgrade_deadline_with_schedule(self):
        ...
        CourseModeFactory(
            ...
            mode_slug=CourseMode.VERIFIED,
            # This must be in the future to ensure it is returned by downstream code.
            expiration_datetime=datetime.datetime.now(get_utc_timezone()) + datetime.timedelta(days=30),
        )
        ...
        Schedule.objects.update(upgrade_deadline=datetime.datetime.now(get_utc_timezone()) + datetime.timedelta(days=5))
    ...
    def test_upgrade_deadline_instructor_paced(self):
        ...
        course_upgrade_deadline = datetime.datetime.now(get_utc_timezone()) + datetime.timedelta(days=1)
        CourseModeFactory(
            course_id=course.id,
            mode_slug=CourseMode.VERIFIED,
            # This must be in the future to ensure it is returned by downstream code.
...
class UserCelebrationTests(SharedModuleStoreTestCase):
    ...
    def test_first_check_streak_celebration(self):
        ...
        today = datetime.datetime.now(get_utc_timezone()).date()
...
class TestUserPostSaveCallback(SharedModuleStoreTestCase):
    ...
    def test_auto_enrollment_if_course_enrollment_closed(self, feature_enabled):
        ...
        enrollment_end = datetime.datetime.now(get_utc_timezone()) - datetime.timedelta(days=2)
