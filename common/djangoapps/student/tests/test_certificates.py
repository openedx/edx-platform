...
PAST_DATE = datetime.datetime.now(get_utc_timezone()) - datetime.timedelta(days=2)
FUTURE_DATE = datetime.datetime.now(get_utc_timezone()) + datetime.timedelta(days=2)
...
class CertificateDashboardMessageDisplayTest(CertificateDisplayTestBase):
    ...
    def _check_message(self, visible_date):  # lint-amnesty, pylint: disable=missing-function-docstring
        ...
        is_past = visible_date < datetime.datetime.now(get_utc_timezone())
