...
def disable_account_ajax(request):
    ...
    try:
        ...
        user_account.standing_last_changed_at = datetime.datetime.now(get_utc_timezone())
        user_account.save()
...
def confirm_email_change(request, key):
    ...
    with transaction.atomic():
        ...
        if 'old_emails' not in meta:
            meta['old_emails'] = []
        meta['old_emails'].append([user.email, datetime.datetime.now(get_utc_timezone()).isoformat()])
