...
def get_user_permissions(user, course_key, org=None):
    ...
    if GlobalStaff().has_user(user) or OrgInstructorRole(org=org).has_user(user):
        return all_perms
    if course_key and has_role(user, CourseInstructorRole(course_key)):
        return all_perms
    # Staff have all permissions except EDIT_ROLES:
    if OrgStaffRole(org=org).has_user(user) or (course_key and has_role(user, CourseStaffRole(course_key))):
        return STUDIO_VIEW_USERS | STUDIO_EDIT_CONTENT | STUDIO_VIEW_CONTENT
    # Otherwise, for libraries, users can view only:
    if course_key and isinstance(course_key, LibraryLocator):
        if OrgLibraryUserRole(org=org).has_user(user) or has_role(user, LibraryUserRole(course_key)):
...
def _check_caller_authority(caller, role):
    ...
    if isinstance(role, (GlobalStaff, CourseCreatorRole)):
        ...
        if not has_role(caller, CourseInstructorRole(role.course_key)):
            raise PermissionDenied
