...
"""
    ...
    def handle(self, *args, **options):
        ...
        if "createlog" in args:
            create_log = True
            file = open('repair_users_roles_on_all_courses.log', 'a+')
        ...
        msg_string += "\nNumber of users with both observer and ta status in same course: {}, number of users that should be removed from global observer group: {}.".format(number_of_conflicted_users, number_of_single_conflicted_users)
        if not dry_run:
            msg_string += "\nAll users roles are cleaned!"
        if create_log:
            file.write(msg_string+'\n')
            file.write('--------------------------------------------------------------------------------------------------------------------\n')
            file.close()
