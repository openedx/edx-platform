...
class Command(BaseCommand):
    ...
    def _create_jd(self, seed_num, grade):
        print (20 * "\n") + ("creating johndoe%s" % seed_num) + (20 * "\n")

        course_id = 'MITx/6.002x/2013_Spring'
        email = 'jd%s@edx.org' % seed_num

        user = User(username='johndoe%s' % seed_num, email='jd%s@edx.org' % seed_num, password='1234')
        user.save()

        profile = UserProfile(user=User.objects.filter(email='jd%s@edx.org' % seed_num)[0], name='John Doe %s' % seed_num)
        profile.save()
    ...
    def _create_n_jds(self, number_to_add, grade):
        ...
        while number_to_add > 0:
            if len(User.objects.filter(username='johndoe%s' % seed_num)) > 0:
                ...
                self._create_jd(seed_num, grade)
                number_to_add -= 1

    def _delete_all_jds(self):
        [user.delete() for user in User.objects.filter(username__contains="johndoe")]
