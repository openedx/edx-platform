...
from student.models import (Registration, UserProfile, TestCenterUser, TestCenterUserForm,
                            ...
                            get_testcenter_registration, CourseEnrollmentAllowed,CourseCategories)
...
from courseware.views import get_module_for_descriptor, jump_to,registered_for_course
...
def _do_create_account(post_vars):
    ...
    profile.area_of_interest = post_vars.get('area_of_interest');
...
def change_interest(request):
    ''' Log a request for change in interest.'''
    if not request.user.is_authenticated:
	raise Http404
    new_interest = request.POST['new_interest']
    cursor = connection.cursor()
    username = str(request.user)
    ...
    for a in row: 
    	if a[0] == user_id:
		query_update = "update auth_userprofile set area_of_interest='"+new_interest+"'"
