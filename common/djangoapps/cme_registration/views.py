...
def cme_create_account(request, post_override=None):
    ...
    for var in ['username', 'email', 'password', 'name']:
        if var not in post_vars:
            jason_string['value'] = "Error (401 {field}). E-mail us.".format(field=var)
            jason_string['field'] = var
            return HttpResponse(json.dumps(jason_string))
    ...
    try:
        ...
        jason_string['value'] = "Valid e-mail is required."
        jason_string['field'] = 'email'
        return HttpResponse(json.dumps(jason_string))
    ...
    try:
        ...
        jason_string['value'] = "Username should only consist of A-Z and 0-9, with no spaces."
        jason_string['field'] = 'username'
        return HttpResponse(json.dumps(jason_string))
    ...
    try:
        ...
        jason_string['value'] = 'Could not send activation e-mail.'
        return HttpResponse(json.dumps(jason_string))
    ...
    jason_string = {'success': True}
    HttpResponse(json.dumps(jason_string), mimetype="application/json")
...
def _do_cme_create_account(post_vars):
    ...
    try:
        ...
        if len(User.objects.filter(username=post_vars['username'])) > 0:
            jason_string['value'] = "An account with the Public Username  '" + post_vars['username'] + "' already exists."
            jason_string['field'] = 'username'
            return HttpResponse(json.dumps(jason_string))

        if len(User.objects.filter(email=post_vars['email'])) > 0:
            jason_string['value'] = "An account with the Email '" + post_vars['email'] + "' already exists."
            jason_string['field'] = 'email'
            return HttpResponse(json.dumps(jason_string))
