...
class CanonicalContentTest(SharedModuleStoreTestCase):
    ...
    def test_canonical_asset_path_with_new_style_assets(self, base_url, start, expected, mongo_calls):
        ...
        encoded_asset_key = urlquote('/asset-v1:a+b+{}+type@asset+block@'.format(prefix))
        ...
        if digest:
            adjusted_asset_key = 'assets/courseware/VMARK/HMARK/asset-v1:a+b+{}+type@asset+block'.format(prefix)
            adjusted_th_key = 'assets/courseware/VMARK/HMARK/asset-v1:a+b+{}+type@thumbnail+block'.format(prefix)
            encoded_asset_key = '/assets/courseware/VMARK/HMARK/asset-v1:a+b+{}+type@asset+block@'.format(prefix)
