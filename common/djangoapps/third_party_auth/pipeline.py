...
def ensure_user_information(
    ...
    **kwargs
    ...
    if dispatch_to_login:
        return redirect(_create_redirect_url(AUTH_DISPATCH_URLS[AUTH_ENTRY_LOGIN], strategy), name='signin_user')
    ...
    if is_register and user_unset:
        return redirect(_create_redirect_url(AUTH_DISPATCH_URLS[AUTH_ENTRY_REGISTER], strategy), name='register_user')

    # TODO (ECOM-369): Consolidate this with `is_register`
    # once the A/B test completes. # pylint: disable=fixme
    if is_register_2 and user_unset:
        return redirect(_create_redirect_url(AUTH_DISPATCH_URLS[AUTH_ENTRY_REGISTER_2], strategy), name='register_user')
