...
class SAMLProviderConfig(ProviderConfig):
    ...
    # validator method on row create
    # looks at entity ID and slug
    # check if any other records exist with that entity ID under a different slug
    # prevents creation of new entry if other records found

    def save(self, *args, **kwargs):
        # Disallowing any new entries that have the same entity ID as an existing provider config
        # Unless the slug matches.
        # This both allows for the old architecture to create new rows on save but also prevents enterprise users from
        # creating new configs that share entity ID's with other enterprises
