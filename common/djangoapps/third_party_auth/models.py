...
from .utils import load_backend_classes, clean_json

log = logging.getLogger(__name__)


_PSA_BACKENDS = {backend_class.name: backend_class for backend_class in load_backend_classes()}
_PSA_OAUTH2_BACKENDS = [backend_class.name for backend_class in load_backend_classes(OAuthAuth)]
_PSA_SAML_BACKENDS = [backend_class.name for backend_class in load_backend_classes(SAMLAuth)]
_LTI_BACKENDS = [backend_class.name for backend_class in load_backend_classes(LTIAuthBackend)]
...
DEFAULT_SAML_ATTRIBUTE_ASSERTIONS = {
...
class SAMLProviderConfig(ProviderConfig):
    ...
    attributes = models.TextField(
        verbose_name="Attributes Assertions", blank=False, default=json.dumps(DEFAULT_SAML_ATTRIBUTE_ASSERTIONS, indent=4),
        help_text="JSON of URNs of SAML attributes containing user data. Leave a value blank to use it's default.")
    ...
    other_settings = models.TextField(
        ...
        help_text=(
            'For advanced use cases, enter a JSON object with additional configuration. '
    ...
    def clean(self):
        ...
        self.other_settings = clean_json(self.other_settings, dict)
        self.attributes = clean_json(self.attributes, dict)
    ...
    def get_config(self):
        ...
        """
        conf = json.loads(self.attributes)
        conf.update({"entity_id": getattr(self, 'entity_id')})
        if self.other_settings:
            conf.update(json.loads(self.other_settings))
