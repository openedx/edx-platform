...
class TransactionIsolationLevelsTestCase(TransactionTestCase):
    ...
    @ddt.data(
        (commit_on_success_with_read_committed, IntegrityError, False, True),
    )
    @ddt.unpack
    def test_concurrent_requests1(self, transaction_decorator, exception_class, created_in_1, created_in_2):
        """
        Test that when isolation level is set to READ COMMITTED get_or_create()
        for the same row in concurrent requests does not raise an IntegrityError.
        """
            ...
            def run(self):
                ...
                try:
                    ...
                    try:
                        ...
                        u = User(username='student', email='student@edx.org')
                ...
                assert len(connection.transaction_state) == 1
                time.sleep(5)
    ...
    @ddt.unpack
    def test_concurrent_requests2(self, transaction_decorator, exception_class, created_in_1, created_in_2):
        """
        Test that when isolation level is set to READ COMMITTED get_or_create()
        for the same row in concurrent requests does not raise an IntegrityError.
        """
