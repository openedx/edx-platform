...
class EntitlementViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        for enrollment in user_run_enrollments:
        ...
        if len(upgradeable_enrollments) == 1:
            log.info('Upgrading enrollment [%s] from audit to [%s] while creating entitlement for user [%s] for course [%s] ', upgradeable_enrollments, serializer.data.get('mode'), user.username, serializer.data.get('course_uuid'))
            ...
            log.info('No enrollment upgraded while creating entitlement for user [%s] for course [%s] ', user.username, serializer.data.get('course_uuid'))
