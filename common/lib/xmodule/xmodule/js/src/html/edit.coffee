class @HTMLEditingDescriptor
    ...
    initInstanceCallback: (visualEditor) =>
        self = this
        onUserChange = (event) ->
            self.onVisualEditorUserInput(event)
        visualEditor.setContent(rewriteStaticLinks(visualEditor.getContent({no_events: 1}), '/static/', @base_asset_url))

        # Unfortunately, just setting visualEditor.isNotDirty = true is not enough to convince TinyMCE we
        # haven't dirtied the Editor. Store the raw content so we can compare it later, and also track whether
        # the user has interacted with the component.
        @starting_content = @getVisualEditorRawContent()
        @seen_user_input = false
        visualEditor.on('keyup', onUserChange)
        visualEditor.on('paste', onUserChange)
        visualEditor.on('drop', onUserChange)

        # Give the editor the focus
        visualEditor.focus()

    onVisualEditorUserInput: (event) =>
    ...
    getVisualEditorRawContent: ->
        return @getVisualEditor().getContent({format:"raw", no_events: 1})

    isVisualEditorDirty: ->
        # We can't trust TinyMCE's isDirty as it returns true for internal rewrites,
        # so this logic attempts to determine if the user has actually made any
        # changes since the editor was initialized.
        if @editor_choice != 'visual'
            return false
        if not @seen_user_input
            return false
        raw_content = @getVisualEditorRawContent()
        return @starting_content != raw_content

    save: ->
        text = undefined
        if @isVisualEditorDirty()
            text = rewriteStaticLinks(@getVisualEditor().getContent({no_events: 1}), @base_asset_url, '/static/')
