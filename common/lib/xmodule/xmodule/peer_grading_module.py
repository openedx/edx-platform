...
class PeerGradingFields(object):
    ...
    due_date = Date(
        help="Due date that should be displayed.", default=None, scope=Scope.settings)
    grace_period_string = String(
        help="Amount of grace to give on the due date.", default=None, scope=Scope.settings)
...
class PeerGradingModule(PeerGradingFields, XModule):
    ...
    js = {
        'coffee': [resource_string(__name__, 'js/src/peergrading/peer_grading.coffee'),
                   resource_string(
                   __name__, 'js/src/peergrading/peer_grading_problem.coffee'),
    ...
    def __init__(self, system, location, descriptor, model_data):
        ...
        if self.use_for_single_location in TRUE_DICT:
            try:
                self.linked_problem = modulestore().get_instance(
                    self.system.course_id, self.link_to_location)
            except:
                log.error("Linked location {0} for peer grading module {1} does not exist".format(
                    self.link_to_location, self.location))
                raise
            due_date = self.linked_problem._model_data.get(
                'peer_grading_due', None)
            if due_date:
        ...
        try:
            ...
            log.error(
                "Error parsing due date information in location {0}".format(location))
            raise
    ...
    def get_score(self):
        ...
        try:
            ...
            if count_required > 0 and count_graded >= count_required:
                # Ensures that once a student receives a final score for peer
                # grading, that it does not change.
                self.student_data_for_location = response

        if self.weight is not None:
            score = int(
                count_graded >= count_required and count_graded > 0) * float(self.weight)
    ...
    def save_grade(self, get):
        ...
        required = set(
            ['location', 'submission_id', 'submission_key', 'score', 'feedback', 'rubric_scores[]',
             'submission_flagged'])
        success, message = self._check_required(get, required)
        if not success:
            return self._err_response(message)
        ...
        try:
            response = self.peer_gs.save_grade(
                location, grader_id, submission_id,
    ...
    def save_calibration_essay(self, get):
        ...
        required = set(['location', 'submission_id',
        ...
        try:
            response = self.peer_gs.save_calibration_essay(
                location, grader_id, calibration_essay_id,
                submission_key, score, feedback, rubric_scores)
            if 'actual_rubric' in response:
                rubric_renderer = combined_open_ended_rubric.CombinedOpenEndedRubric(
                    self.system, True)
                response['actual_rubric'] = rubric_renderer.render_rubric(
                    response['actual_rubric'])['html']
            return response
        except GradingServiceError:
            # This is a dev_facing_error
            log.exception(
    ...
    def peer_grading(self, get=None):
        ...
        problem_list = []
        try:
            problem_list_json = self.peer_gs.get_problem_list(
                self.system.course_id, self.system.anonymous_student_id)
        ...
        def _find_corresponding_module_for_location(location):
            ...
            try:
                ...
                log.error("Problem {0} does not exist in this course".format(
                    location))
                raise

        for problem in problem_list:
            problem_location = problem['location']
            descriptor = _find_corresponding_module_for_location(
                problem_location)
            if descriptor:
                problem['due'] = descriptor._model_data.get(
                    'peer_grading_due', None)
                grace_period_string = descriptor._model_data.get(
                    'graceperiod', None)
                try:
                    problem_timeinfo = TimeInfo(problem[
                                                'due'], grace_period_string)
                except:
                    log.error("Malformed due date or grace period string for location {0}".format(
                        problem_location))
                    raise
                ...
                # if we can't find the due date, assume that it doesn't have
                # one
                problem['due'] = None
                problem['closed'] = False
...
class PeerGradingDescriptor(PeerGradingFields, RawDescriptor):
    ...
    def non_editable_metadata_fields(self):
        non_editable_fields = super(
            PeerGradingDescriptor, self).non_editable_metadata_fields
        non_editable_fields.extend(
            [PeerGradingFields.due_date, PeerGradingFields.grace_period_string,
