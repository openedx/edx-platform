    ...
    def initdb(cls):
        ...
        with patch('xmodule.tabs.CourseTab.from_json', side_effect=mock_tab_from_json):
            ...
            import_course_from_xml(
                ...
                do_import_static=False,
                verbose=True,
                target_id=CourseLocator('guestx', 'foo', 'bar')
            )
...
class TestMongoModuleStore(TestMongoModuleStoreBase):
    ...
    def test_get_courses_for_wiki(self, _from_json):
        ...
        for course_number in self.courses:
            ...
            assert_equals(CourseLocator('edX', course_number, '2012_Fall'), course_locations[0])
        ...
        toy_course = self.draft_store.get_course(CourseLocator('edX', 'toy', '2012_Fall'))
        ...
        for course_number in ['toy', 'simple']:
            assert_in(CourseLocator('edX', course_number, '2012_Fall'), course_locations)

        # configure simple course to use unique wiki_slug.
        simple_course = self.draft_store.get_course(CourseLocator('edX', 'simple', '2012_Fall'))
        ...
        assert_equals(len(course_locations), 1)
        assert_in(CourseLocator('edX', 'simple', '2012_Fall'), course_locations)
    ...
    def test_export_course_image(self, _from_json):
        ...
        """
        course_key = CourseLocator('edX', 'simple', '2012_Fall')
    ...
    def test_export_course_image_nondefault(self, _from_json):
        ...
        """
        course = self.draft_store.get_course(CourseLocator('edX', 'toy', '2012_Fall'))
    ...
    def test_course_without_image(self):
        ...
        image
        """
        course = self.draft_store.get_course(CourseLocator('edX', 'simple_with_draft', '2012_Fall'))
        root_dir = path(mkdtemp())
        self.addCleanup(shutil.rmtree, root_dir)
        export_course_to_xml(self.draft_store, self.content_store, course.id, root_dir, 'test_export')
        self.assertFalse(path(root_dir / 'test_export/static/images/course_image.jpg').isfile())
    ...
    def _create_test_tree(self, name, user_id=None):
        ...
        if not self.draft_store.has_course(CourseLocator(org, course, run)):
