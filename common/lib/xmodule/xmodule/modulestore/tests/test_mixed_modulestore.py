...
class TestMixedModuleStore(LocMapperSetupSansDjango):
    ...
    @ddt.data('draft')
    def test_revert_to_published_root_direct_only(self, default_ms):
        """
        Test calling revert_to_published on direct-only sequential.
        """
        self.initdb(default_ms)
        self._create_block_hierarchy()
        self._revert_to_published(self.sequential_x1.location, default_ms)
    ...
    def test_revert_to_published_root_draft(self, default_ms):
        ...
        self.initdb(default_ms)
        self._create_block_hierarchy()
        self._revert_to_published(self.vertical_x1a.location, default_ms)

    def _revert_to_published(self, root_location, default_ms):
        """
        Helper method for testing revert_to_published.
        """
        # publish the course
        self.store.publish(self.course.location, self.user_id)
        ...
        self.store.revert_to_published(root_location, self.user_id)
