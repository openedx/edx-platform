...
class SplitMongoModuleStore(ModuleStoreWriteBase):
    ...
    def _lookup_course(self, course_locator):
        ...
        # b/c more than one course can use same structure, the 'org', 'course', 'run', and 'branch' are not intrinsic to structure
    ...
    def create_item(
        ...
        * If the course locator specifies a org, course, and run and either it doesn't
        ...
        * If the locator has a org, course and run but its version_guid != current head, it raises VersionConflictError.
    ...
    def create_course(
        ...
        If a course with the same org, course, and run already exists, this method will raise DuplicateCourseError
    ...
    def update_item(self, descriptor, user_id, allow_not_found=False, force=False):
        ...
        Creates a new course version. If the descriptor's location has a org, course and run, it moves the course head
    ...
    def _get_index_if_valid(self, locator, force=False, continue_version=False):
        ...
        """
        if None in (locator.org, locator.course, locator.run, locator.branch):
            if continue_version:
