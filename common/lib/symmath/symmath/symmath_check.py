...
#
# Takes in math expressions given as Presentation MathML (from
# ASCIIMathML), converts to Content MathML using SnuggleTeX
...
def check(expect, given, numerical=False, matrix=False, normphase=False, abcsym=False, do_qubit=True, symtab=None, dosimplify=False):
    ...
    if "__or__" in expect:			# if multiple acceptable answers
        ...
        for eone in eset:
            ret = check(eone, given, numerical, matrix,
                        normphase, abcsym, do_qubit, symtab, dosimplify)
            if ret['ok']:
                return ret
        return ret
    ...
    # msg += "<p/> dot test " +
    # to_latex(dot(sympy.Symbol('x'),sympy.Symbol('y')))
...
def make_error_message(msg):
    # msg = msg.replace('<p>','<p><span class="inline-
    # error">').replace('</p>','</span></p>')
...
def symmath_check(expect, ans, dynamath=None, options=None, debug=None, xml=None):
    ...
    if xml is not None:
        DEBUG = xml.get(
            'debug', False)  	# override debug flag using attribute in symbolicmath xml
        if DEBUG in ['0', 'False']:
            DEBUG = False
    ...
    try:
        ...
        msg += '<p>Error %s in parsing OUR expected answer "%s"</p>' % (
            err, expect)
        return {'ok': False, 'msg': make_error_message(msg)}

    ###### Sympy input #######
    # if expected answer is a number, try parsing provided answer as a number
    # also
    try:
    ...
    if do_numerical:		# numerical answer expected - force numerical comparison
        if is_within_tolerance(fexpect, fans, threshold):
            ...
            msg += '<p>You entered: %s (note that a numerical answer is expected)</p>' % to_latex(
                fans)
            return {'ok': False, 'msg': msg}
    ...
    try:
        ...
        log.exception(
            "Error evaluating expression '%s' as a valid equation" % ans)
    ...
    if type(fexpect) == list:
        try:
            ...
            dm = my_evalf(sympy.Matrix(
                fexpect) - sympy.Matrix(xgiven), chop=True)
            if abs(dm.vec().norm().evalf()) < threshold:
                return {'ok': True, 'msg': msg}
            ...
            msg += "<p>Error %s in comparing expected (a list) and your answer</p>" % str(
                err).replace('<', '&lt;')
            if DEBUG:
    ...
    if DEBUG:
        ...
        # msg += "<p/>Got: %s" % str([type(x) for x in
        # fsym.atoms()]).replace('<','&lt;')
        msg += "<p>Expecting: %s</p>" % repr(fexpect).replace(
            '**', '^').replace('hat(I)', 'hat(i)')
        # msg += "<p/>Expecting: %s" % str([type(x) for x in
        # fexpect.atoms()]).replace('<','&lt;')
        if diff:
