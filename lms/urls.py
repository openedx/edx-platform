...
import student
import util
from branding.views import courses as branding_courses
from branding.views import index as branding_index
from certificates.views import request_certificate, update_certificate, update_example_certificate
from config_models.views import ConfigurationModelCurrentAPIView
from course_wiki.views import course_wiki_redirect, root_create
...
from debug.views import run_python, show_parameters
from django_comment_common.models import ForumsConfig
from django_openid_auth.views import login_begin, logo
from lms.djangoapps.discussion.views import course_discussions_settings_handler, discussion_topics
from lms.djangoapps.instructor.views.coupons import add_coupon, get_coupon_info, remove_coupon, update_coupon
from lms.djangoapps.instructor.views.instructor_dashboard import instructor_dashboard_2, set_course_mode_price
from lms.djangoapps.instructor.views.registration_codes import look_up_registration_code, registration_code_details
from lms.djangoapps.instructor_task.views import instructor_task_status
from lms_migration.migrate import gitreload, manage_modulestores
from notes.views import notes
from notification_prefs.views import ajax_disable, ajax_enable, ajax_status, set_subscription
from openedx.core.djangoapps.auth_exchange.views import LoginWithAccessTokenView
from openedx.core.djangoapps.catalog.models import CatalogIntegration
from openedx.core.djangoapps.common_views.xblock import xblock_resource
from openedx.core.djangoapps.cors_csrf.views import xdomain_proxy
...
from openedx.core.djangoapps.verified_track_content.views import cohorting_settings
from openedx.features.enterprise_support.api import enterprise_enabled
from staticbook.views import html_index as staticbook_html_index
from staticbook.views import index as staticbook_index
from staticbook.views import pdf_index as staticbook_pdf_index
from student.views import login_oauth_token
from student_account.views import login_and_registration_form
from track.views import view_tracking_log
...
if settings.FEATURES["ENABLE_OPENBADGES"]:
    urlpatterns += [
        url(r'^api/badges/v1/', include('badges.api.urls', namespace="badges_api")),
    ]
...
if settings.WIKI_ENABLED:
    ...
    urlpatterns += [
        ...
        url(r'^wiki/create-root/$', root_create, name='root_create'),
        ...
        url(r'^courses/{}/course_wiki/?$'.format(settings.COURSE_ID_PATTERN),
            course_wiki_redirect, name="course_wiki"),
...
if settings.FEATURES.get('ENABLE_DISCUSSION_SERVICE'):
    urlpatterns += [
        ...
        url(r'^courses/{}/discussion/'.format(settings.COURSE_ID_PATTERN), include('django_comment_client.urls')),
        url(r'^courses/{}/discussion/forum/'.format(settings.COURSE_ID_PATTERN), include('discussion.urls')),
        url(r'^notification_prefs/enable/', ajax_enable),
...
urlpatterns += [
    url(r'^courses/{}/tab/(?P<tab_type>[^/]+)/$'.format(settings.COURSE_ID_PATTERN), CourseTabView.as_view(),
        name='course_tab_view'),
]

urlpatterns += [
    # This MUST be the last view in the courseware--it's a catch-all for custom tabs.
    url(r'^courses/{}/(?P<tab_slug>[^/]+)/$'.format(settings.COURSE_ID_PATTERN), StaticCourseTabView.as_view(),
        name='static_tab'),
]
...
if settings.FEATURES.get('AUTH_USE_OPENID'):
    urlpatterns += [
        ...
        url(r'^openid/complete/$', openid_login_complete, name='openid-complete'),
        url(r'^openid/logo.gif$', logo, name='openid-logo'),
    ]

if settings.FEATURES.get('AUTH_USE_SHIB'):
    urlpatterns += [
        url(r'^shib-login/$', shib_login, name='shib-login')
    ]
...
if settings.FEATURES.get('RESTRICT_ENROLL_BY_REG_METHOD'):
    urlpatterns += [
        url(r'^course_specific_login/{}/$'.format(settings.COURSE_ID_PATTERN),
            course_specific_login, name='course-specific-login'),
        url(r'^course_specific_register/{}/$'.format(settings.COURSE_ID_PATTERN),
            course_specific_register, name='course-specific-register'),
    ]
...
if settings.FEATURES.get('ENABLE_LMS_MIGRATION'):
    urlpatterns += [
        url(r'^migrate/modules$', manage_modulestores),
        url(r'^migrate/reload/(?P<reload_dir>[^/]+)$', manage_modulestores),
        url(r'^migrate/reload/(?P<reload_dir>[^/]+)/(?P<commit_id>[^/]+)$', manage_modulestores),
        url(r'^gitreload$', gitreload),
        url(r'^gitreload/(?P<reload_dir>[^/]+)$', gitreload),
    ]
...
if settings.FEATURES.get('ENABLE_DEBUG_RUN_PYTHON'):
    urlpatterns += [
        url(r'^debug/run_python$', run_python),
    ]

urlpatterns += [
    url(r'^debug/show_parameters$', show_parameters),
]
...
if settings.FEATURES.get('ENABLE_THIRD_PARTY_AUTH'):
    urlpatterns += [
        ...
        url(r'^login_oauth_token/(?P<backend>[^/]+)/$', login_oauth_token),
    ]
...
# XDomain proxy
urlpatterns += [
    url(r'^xdomain_proxy.html$', xdomain_proxy, name='xdomain_proxy'),
]
