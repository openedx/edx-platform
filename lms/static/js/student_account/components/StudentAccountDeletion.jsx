...
export class StudentAccountDeletion extends React.Component {
  ...
  render() {
    ...
    const changeAcctInfoText = StringUtils.interpolate(
      ...
      {
        htmlStart: '<a href="https://edtech.ucsd.edu/uc-san-diego-online-help/manage-your-account-settings" target="_blank">',
        htmlEnd: '</a>',
      },
    );

    const acctDeletionWarningText = StringUtils.interpolate(
      gettext('{strongStart}Warning: Account deletion is permanent.{strongEnd} Please read the above carefully before proceeding. This is an irreversible action, and {strongStart}you will no longer be able to use the same email on UC San Diego Online.{strongEnd}'),
      {
        strongStart: '<strong>',
        strongEnd: '</strong>',
      },
    ...
    return (
      <div className="account-deletion-details">
        ...
        <p className="account-settings-header-subtitle">{ gettext('Please note: Deletion of your account and personal data is permanent and cannot be undone. UC San Diego Online will not be able to recover your account or the data that is deleted.') }</p>
        <p className="account-settings-header-subtitle">{ gettext('Once your account is deleted, you cannot use it to access or take courses through UC San Diego Online.') }</p>
        <p
