...
class BlocksView(DeveloperErrorViewMixin, ListAPIView):
    ...
    **Parameters**:
        ...
        * requested_fields: (list) Indicates which additional fields to return for each block.
          For a list of available fields see under `Response Values -> blocks`, below.
...
class BlocksInCourseView(BlocksView):
    ...
    **Parameters**:
        ...
        * student_view_data: (list) Indicates for which block types to return
          student_view_data.  See `Response Values -> blocks -> type` below
          for valid block types.

          Example: student_view_data=video

        * block_counts: (list) Indicates for which block types to return the
          aggregate count of the blocks.  See `Response Values -> blocks ->
          type` below for valid block types.

          Example: block_counts=video,problem

        * requested_fields: (list) Indicates which additional fields to return
          for each block.  See `Response Values -> blocks` below for valid
          fields.
        ...
        * depth (integer or all) Indicates how deep to traverse into the blocks
          hierarchy.  Must be a non-negative integer or "all".  A value of all
          means the entire hierarchy.
