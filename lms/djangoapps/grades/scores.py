...
def get_score(submissions_scores, csm_scores, persisted_block, block):
    ...
    if str(block.location.course_key) == 'course-v1:UQx+BUSLEAD5x+2T2019':
        log.info(u'Weight for block: ***{}*** is {}'  # lint-amnesty, pylint: disable=logging-format-interpolation
    ...
    # TODO: Remove as part of EDUCATOR-4602.
    if str(block.location.course_key) == 'course-v1:UQx+BUSLEAD5x+2T2019':
        log.info(u'Calculated raw-earned: {}, raw_possible: {}, weighted_earned: '  # lint-amnesty, pylint: disable=logging-format-interpolation
...
def _get_score_from_persisted_or_latest_block(persisted_block, block, weight):
    ...
    # TODO: Remove as part of EDUCATOR-4602.
    if str(block.location.course_key) == 'course-v1:UQx+BUSLEAD5x+2T2019':
        log.info(u'Using _get_score_from_persisted_or_latest_block to calculate score for block: ***{}***.'.format(  # lint-amnesty, pylint: disable=logging-format-interpolation
            str(block.location)
        ))
    raw_earned = 0.0
    first_attempted = None
    ...
    if persisted_block:
        ...
        if str(block.location.course_key) == 'course-v1:UQx+BUSLEAD5x+2T2019':
            ...
            log.info(u'weight for block: ***{}*** is {}.'.format(str(block.location), raw_possible))  # lint-amnesty, pylint: disable=logging-format-interpolation
