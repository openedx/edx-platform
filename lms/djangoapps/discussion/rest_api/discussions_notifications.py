...
class DiscussionNotificationSender:
    ...
    def _get_cohort_course_audience_queryset(self) -> BatchNotificationAudience:
        """
        Returns queryset for notification audience based on user cohort and role
        """
        ...
        batch_audience = BatchNotificationAudience(exclude=[self.creator.id])

        if team:
            user_ids = team.users.all().values_list('id', flat=True)
            batch_audience.add_queryset(user_ids)
            ...
            ).values('users').annotate(id=F('users__id')).values_list('id', flat=True)
            batch_audience.add_queryset(users_in_cohort)
            ...
            privileged_users = Role.objects.filter(
                name__in=privileged_roles, course_id=course_key_str
            ).values('users').annotate(id=F('users__id')).values_list('id', flat=True)
            batch_audience.add_queryset(privileged_users)

            staff_users = CourseStaffRole(self.course.id).users_with_role().values_list('id', flat=True)
            batch_audience.add_queryset(staff_users)

            admin_users = CourseInstructorRole(self.course.id).users_with_role().values_list('id', flat=True)
            batch_audience.add_queryset(admin_users)
        else:
            user_ids = CourseEnrollment.objects.filter(
                course__id=course_key_str, is_active=True
            ).values_list('user__id', flat=True)
            batch_audience.add_queryset(user_ids)
        return batch_audience

    def _create_cohort_course_audience(self):
        """
        Returns audience as list for cohorted courses
        """
        return list(self._get_cohort_course_audience_queryset().get_queryset())

    def _send_new_thread_created_notification_in_batch(self, batch_ids):
        """
        Creates new thread created notification for a batch
        """
        ...
        self._send_notification(batch_ids, notification_type, context)

    def send_new_thread_created_notification(self):
        """
        Send notification based on notification_type
        """
        batch_audience = self._get_cohort_course_audience_queryset()
        batch_audience.set_callback(self._send_new_thread_created_notification_in_batch)
        batch_audience.generate_audience()
