    ...
    if form_value:
        ...
        if reported_content_email_notification_enabled(CourseKey.from_string(cc_content.course_id)):
            if cc_content.type == 'thread':
                thread_flagged.send(sender='flag_abuse_for_thread', user=user, post=cc_content, reported_content=True)
            else:
                comment_flagged.send(sender='flag_abuse_for_comment', user=user, post=cc_content, reported_content=True)
    else:
