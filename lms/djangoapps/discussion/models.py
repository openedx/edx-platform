...
class DiscussionModerationLog(models.Model):
    """
    Audit log for discussion moderation actions.
    ...
    ACTION_CHOICES = [
        (ACTION_BAN, _('Ban User')),
        (ACTION_UNBAN, _('Unban User')),
        (ACTION_BAN_EXCEPTION, _('Ban Exception Created')),
        (ACTION_BULK_DELETE, _('Bulk Delete')),
    ]
    ...
    reason = models.TextField(null=True, blank=True)
    metadata = models.JSONField(null=True, blank=True)  # Task IDs, counts, etc.
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['target_user', '-created'], name='idx_target_user'),
            models.Index(fields=['moderator', '-created'], name='idx_moderator'),
            models.Index(fields=['course_id', '-created'], name='idx_course'),
            models.Index(fields=['action_type', '-created'], name='idx_action_type'),
        ]
        verbose_name = _('Discussion Moderation Log')
        verbose_name_plural = _('Discussion Moderation Logs')

    def __str__(self):
        moderator_name = self.moderator.username if self.moderator else 'System'
        return f"{self.action_type}: {self.target_user.username} by {moderator_name}"
