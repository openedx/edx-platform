...
class OrderTest(ModuleStoreTestCase):
    ...
    def test_purchase(self):
        ...
        self.assertIn(str(cart.total_cost), mail.outbox[0].body)
        ...
        self.mock_tracker.track.assert_called_once_with(
            ...
            'Completed Order',
            {
                ...
                'products': [
                    {
                        'sku': 'CertificateItem.honor',
                        'name': str(self.course_key),
                        'category': str(self.course_key.org),
                        'price': '40.00',
                        'id': 1,
                        'quantity': 1
                    }
...
class CertificateItemTest(ModuleStoreTestCase):
    ...
    def test_no_id_prof_confirm_email(self):
        ...
        cart.purchase()
        enrollment = CourseEnrollment.objects.get(user=self.user, course_id=self.course_key)
        self.assertEqual(enrollment.mode, 'no-id-professional')
...
class DonationTest(ModuleStoreTestCase):
    ...
    def test_donate_to_org(self):
        ...
        self._assert_donation(
            ...
            unit_cost=self.COST,
            line_desc="Donation for {}".format(settings.PLATFORM_NAME)
        )
    ...
    def test_donate_to_course(self):
        ...
        self._assert_donation(
            ...
            course_id=course.id,
            unit_cost=self.COST,
            line_desc="Donation for Test Course"
        )
