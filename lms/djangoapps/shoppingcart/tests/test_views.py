...
class ShoppingCartViewsTests(SharedModuleStoreTestCase, XssTestMixin):
    ...
    def test_soft_delete_coupon(self):
        ...
        coupon.save()
        self.assertEqual(coupon.__unicode__(), '[Coupon] code: TestCode course: MITx/999/Robot_Super_Course')
    ...
    def test_show_receipt_json(self, num_items):
        ...
        for item in json_resp.get('items'):
            self.assertEqual(item, {
                ...
                'course_key': str(self.verified_course_key)
            })
    ...
    def test_show_receipt_success_with_order_type_business(self):
        ...
        self.assertIn("Invoice #{order_id}".format(order_id=self.cart.id), resp.content.decode(resp.charset))
        codes_string = 'You have successfully purchased <b>{total_registration_codes} course registration codes'
...
class ShoppingcartViewsClosedEnrollment(ModuleStoreTestCase):
    ...
    def test_to_check_that_cart_item_enrollment_is_closed_when_clicking_the_payment_button(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertIn(
            "{course_name} has been removed because the enrollment period has closed.".format(
