...
class Command(BaseCommand):
    ...
    def handle(self, program_uuid, user_items, *args, **options):
        ...
        program_enrollment_user_tuples = self.build_program_enrollment_user_tuples(
        ...
        with transaction.atomic():
            for program_enrollment, user in program_enrollment_user_tuples:
                self.link_program_enrollment(program_enrollment, user)
                # self.do_courses_now(program_enrollment)
    ...
    def parse_user_items(self, user_items):
        ...
        for user_item in user_items:
            ...
            if len(split_args) != 2:
                message = (u'incorrectly formatted argument {}, '
                           'must be in form <external user key>:<lms username>').format(user_item)
                raise CommandError(message)

            external_user_key = split_args[0]
            lms_username = split_args[1]
            if external_user_key in result:
                raise CommandError(u'external user key {} provided multiple times'.format(external_user_key))
    ...
    def build_program_enrollment_user_tuples(self, ext_keys_to_usernames, program_uuid, program_enrollments, users):
        result = []
        for external_student_key, lms_username in ext_keys_to_usernames.items():
            ...
            user = users.get(lms_username)
            if not user:
                logger.warning(NO_LMS_USER_TPL.format(lms_username))
                continue
            result.append((program_enrollment, user))
        return result
    ...
    def link_program_enrollment(self, program_enrollment, user):
        ...
        for program_course_enrollment in program_enrollment.program_course_enrollments.all():
            try:
                ...
                logger.warning(u'Failed to enroll user {} with waiting program course enrollment for course {}'.format(
                    user.username,
    ...
    def log_existing_user_message(self, program_enrollment, user):
        ...
        message = u'Program enrollment with external_student_key={} is already linked to {} account username={}'
