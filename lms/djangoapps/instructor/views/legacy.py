...
def instructor_dashboard(request, course_id):
    ...
    # DataDump
        ...
        datatable['data'] = [[s.id, unique_id_for_user(s, save=False), anonymous_id_for_user(s, course_id, save=False)] for s in students]
    ...
    # enrollment
        ...
        ret = _do_enroll_students(course, course_id, students, secure=secure, auto_enroll=auto_enroll, email_students=email_students, is_shib_course=is_shib_course)
        ...
        if not 'List' in action:
            ...
            secure = request.is_secure()
            ret = _do_enroll_students(course, course_id, students, secure=secure, overload=overload)
            datatable = ret['datatable']
...
def _do_enroll_students(course, course_id, students, secure=False, overload=False, auto_enroll=False, email_students=False, is_shib_course=False):
    """
    ...
    if email_students:
        ...
        course_url = '{proto}://{site}{path}'.format(
            proto=protocol,
            site=stripped_site_name,
            path=reverse('course_root', kwargs={'course_id': course_id})
        )
        ...
        if not settings.FEATURES.get('ENABLE_MKTG_SITE', False):
            course_about_url = u'{proto}://{site}{path}'.format(
                proto=protocol,
                site=stripped_site_name,
                path=reverse('about_course', kwargs={'course_id': course_id})
            )
