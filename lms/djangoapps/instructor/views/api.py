...
def get_anon_ids(request, course_id):  # pylint: disable=W0613
    ...
    return csv_response(unicode(course_id).replace('/', '-') + '-anon-ids.csv', header, rows)
...
def get_student_progress_url(request, course_id):
    ...
    progress_url = reverse('student_progress', kwargs={'course_id': unicode(course_id), 'student_id': user.id})
...
def _display_unit(unit):
    ...
    if name:
        return u'{0} ({1})'.format(name, unicode(unit.location))
    else:
        return unicode(unit.location)
