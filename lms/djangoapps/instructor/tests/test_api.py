...
class TestInstructorAPIDenyLevels(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def _access_endpoint(self, endpoint, args, status_code, msg):
        ...
        """
        url = reverse(endpoint, kwargs={'course_id': unicode(self.course.id)})
...
class TestInstructorAPIBulkBetaEnrollment(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_missing_params(self):
        """ Test missing all query parameters. """
        url = reverse('bulk_beta_modify_access', kwargs={'course_id': unicode(self.course.id)})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 400)
...
class TestInstructorAPILevelsAccess(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_modify_access_noparams(self):
        """ Test missing all query parameters. """
        url = reverse('modify_access', kwargs={'course_id': unicode(self.course.id)})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 400)
    ...
    def test_list_course_role_members_noparams(self):
        """ Test missing all query parameters. """
        url = reverse('list_course_role_members', kwargs={'course_id': unicode(self.course.id)})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 400)
    ...
    def assert_update_forum_role_membership(self, unique_student_identifier, rolename, action):
        ...
        url = reverse('update_forum_role_membership', kwargs={'course_id': unicode(self.course.id)})
...
class TestInstructorAPILevelsDataDump(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_get_anon_ids(self):
        ...
        """
        url = reverse('get_anon_ids', kwargs={'course_id': unicode(self.course.id)})
        response = self.client.get(url, {})
    ...
    def test_list_report_downloads(self):
        url = reverse('list_report_downloads', kwargs={'course_id': unicode(self.course.id)})
    ...
    def test_calculate_grades_csv_success(self):
        url = reverse('calculate_grades_csv', kwargs={'course_id': unicode(self.course.id)})
    ...
    def test_get_distribution_no_feature(self):
        ...
        """
        url = reverse('get_distribution', kwargs={'course_id': unicode(self.course.id)})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        res_json = json.loads(response.content)
    ...
    def test_get_student_progress_url(self):
        ...
        url = reverse('get_student_progress_url', kwargs={'course_id': unicode(self.course.id)})
...
class TestInstructorAPIRegradeTask(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_reset_student_attempts_deletall(self):
        ...
        url = reverse('reset_student_attempts', kwargs={'course_id': unicode(self.course.id)})
        response = self.client.get(url, {
    ...
    def test_rescore_problem_single(self, act):
        ...
        url = reverse('rescore_problem', kwargs={'course_id': unicode(self.course.id)})
        response = self.client.get(url, {
            'problem_to_reset': self.problem_urlname,
            'unique_student_identifier': self.student.email,
        })
...
class TestInstructorSendEmail(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_send_email_as_logged_in_instructor(self):
        url = reverse('send_email', kwargs={'course_id': unicode(self.course.id)})
...
class TestInstructorAPITaskLists(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_list_instructor_tasks_running(self, act):
        ...
        url = reverse('list_instructor_tasks', kwargs={'course_id': unicode(self.course.id)})
        mock_factory = MockCompletionInfo()
    ...
    def test_list_background_email_tasks(self, act):
        ...
        act.return_value = self.tasks
        url = reverse('list_background_email_tasks', kwargs={'course_id': unicode(self.course.id)})
        mock_factory = MockCompletionInfo()
        with patch('instructor.views.api.get_task_completion_info') as mock_completion_info:
            mock_completion_info.side_effect = mock_factory.mock_get_task_completion_info
            response = self.client.get(url, {})
...
class TestInstructorAPIAnalyticsProxy(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_analytics_proxy_url(self, act):
        ...
        url = reverse('proxy_legacy_analytics', kwargs={'course_id': unicode(self.course.id)})
        response = self.client.get(url, {
...
class TestDueDateExtensions(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def setUp(self):
        ...
        course.children = [unicode(week1.location), unicode(week2.location),
                           unicode(week3.location)]

        homework = ItemFactory.create(
            parent_location=week1.location,
            due=due
        )
        week1.children = [unicode(homework.location)]
    ...
    def test_change_due_date(self):
        url = reverse('change_due_date', kwargs={'course_id': unicode(self.course.id)})
        response = self.client.get(url, {
            'student': self.user1.username,
            'url': unicode(self.week1.location),
    ...
    def test_reset_date(self):
        ...
        url = reverse('reset_due_date', kwargs={'course_id': unicode(self.course.id)})
        response = self.client.get(url, {
            'student': self.user1.username,
            'url': unicode(self.week1.location),
        })
    ...
    def test_show_unit_extensions(self):
        ...
        response = self.client.get(url, {'url': unicode(self.week1.location)})
        self.assertEqual(response.status_code, 200, response.content)
