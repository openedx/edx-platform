...
class TestInstructorDashboardForumAdmin(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_add_and_remove_forum_admin_users(self):
        ...
        for rolename in FORUM_ROLES:
            ...
            self.assertContains(response, 'Added "{0}" to "{1}" forum role = "{2}"'.format(username, unicode(course.id), rolename))
            ...
            self.assertContains(response, 'Removed "{0}" from "{1}" forum role = "{2}"'.format(username, unicode(course.id), rolename))
