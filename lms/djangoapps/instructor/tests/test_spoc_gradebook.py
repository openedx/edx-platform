...
class TestDefaultGradingPolicy(TestGradebook):
    ...
    def test_default_policy(self):
        ...
        self.assertEqual(7, self.response.content.count('grade_Pass'))
        ...
        self.assertEqual(22, self.response.content.count('grade_F'))

        # All other grades are None [29 categories * 11 users - 27 non-empty grades = 292]
        # One use at the top of the page [1]
        self.assertEqual(293, self.response.content.count('grade_None'))
...
class TestLetterCutoffPolicy(TestGradebook):
    ...
    def test_styles(self):
        ...
        self.assertIn("grade_A {color:green;}", self.response.content.decode(self.response.charset))
        self.assertIn("grade_B {color:Chocolate;}", self.response.content.decode(self.response.charset))
        self.assertIn("grade_C {color:DarkSlateGray;}", self.response.content.decode(self.response.charset))
        self.assertIn("grade_D {color:DarkSlateGray;}", self.response.content.decode(self.response.charset))

    def test_assigned_grades(self):
        # Users 9-10 have >= 90% on Homeworks [2]
        # Users 9-10 have >= 90% on the class [2]
        # One use at the top of the page [1]
        self.assertEqual(5, self.response.content.count('grade_A'))

        # User 8 has 80 <= Homeworks < 90 [1]
        # User 8 has 80 <= class < 90 [1]
        # One use at the top of the page [1]
        self.assertEqual(3, self.response.content.count('grade_B'))

        # User 7 has 70 <= Homeworks < 80 [1]
        # User 7 has 70 <= class < 80 [1]
        # One use at the top of the page [1]
        self.assertEqual(3, self.response.content.count('grade_C'))
        ...
        # One use at top of the page [1]
        self.assertEqual(11, self.response.content.count('grade_F'))

        # User 0 has 0 on Homeworks [1]
        # User 0 has 0 on the class [1]
        # One use at the top of the page [1]
        self.assertEqual(3, self.response.content.count('grade_None'))
