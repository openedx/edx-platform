...
class TestDefaultGradingPolicy(TestGradebook):
    ...
    def test_default_policy(self):
        ...
        self.assertEquals(7, self.response.content.count(b'grade_Pass'))
        ...
        self.assertEquals(22, self.response.content.count(b'grade_F'))

        # All other grades are None [29 categories * 11 users - 27 non-empty grades = 292]
        # One use at the top of the page [1]
        self.assertEquals(293, self.response.content.count(b'grade_None'))
...
class TestLetterCutoffPolicy(TestGradebook):
    ...
    def test_assigned_grades(self):
        ...
        # One use at the top of the page [1]
        self.assertEquals(5, self.response.content.count(b'grade_A'))

        # User 8 has 80 <= Homeworks < 90 [1]
        # User 8 has 80 <= class < 90 [1]
        # One use at the top of the page [1]
        self.assertEquals(3, self.response.content.count(b'grade_B'))

        # User 7 has 70 <= Homeworks < 80 [1]
        # User 7 has 70 <= class < 80 [1]
        # One use at the top of the page [1]
        self.assertEquals(3, self.response.content.count(b'grade_C'))
        ...
        # One use at top of the page [1]
        self.assertEquals(11, self.response.content.count(b'grade_F'))

        # User 0 has 0 on Homeworks [1]
        # User 0 has 0 on the class [1]
        # One use at the top of the page [1]
        self.assertEquals(3, self.response.content.count(b'grade_None'))
