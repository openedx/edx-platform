...
def strip_time(date_val):
    """
    Removes time part of datetime
    """
    tzinfo = getattr(date_val, 'tzinfo', now().tzinfo) or now().tzinfo
...
def to_mysql_datetime(date_val):
    """
    convert python datetime to mysql compatible datetime
    """
    return datetime.datetime.strftime(date_val, '%Y-%m-%d %H:%M:%S')
