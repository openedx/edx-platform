...
from server_api.models import GroupProfile, APIUser as User
from server_api.util.courseware_access import get_course, get_course_child, get_course_key, course_exists
from server_api.util.permissions import SecureAPIView, SecureListAPIView, IdsInFilterBackend, HasOrgsFilterBackend
from server_api.util.utils import generate_base_uri, dict_has_items, extract_data_params
...
def _save_content_position(request, user, course_key, position):
    ...
    # no need to fetch the actual child descriptor (avoid round trip to
    # Mongo database), we just need the id
    child_key = None
    try:
...
class UsersList(SecureListAPIView):
    ...
    * Values for level_of_education can be found in the LEVEL_OF_EDUCATION_CHOICES enum, located in common/student/models.py
    ...
    def _create_user_profile(self, request, user):
        """
        Helper method to contain UserProfile creation workflow
        (makes pylint happy)
        """
...
class UsersDetail(SecureAPIView):
    ...
    * A GET response will additionally include a list of URIs to available sub-resources:
    ...
    def _update_username(self, request, existing_user, response_data):
        """
        Helper method to contain username update logic (makes pylint happy)
        """
        ...
        if username:
            ...
            existing_username = User.objects.filter(username=username).filter(~Q(id=existing_user.id))
    ...
    def _update_password(self, request, existing_user, response_data):
        """
        Helper method to contain password update logic (makes pylint happy)
        """
        ...
        if password:
            ...
            if not PasswordHistory.is_allowable_password_reuse(existing_user, password):
                ...
                err_msg = 'You are re-using a password that you have used recently. You must have {} distinct password(s) before reusing a previous password.'.format(num_distinct)

            # also, check to see if passwords are getting reset too frequent
            if PasswordHistory.is_password_reset_too_soon(existing_user):
                num_days = settings.ADVANCED_SECURITY_CONFIG['MIN_TIME_IN_DAYS_BETWEEN_ALLOWED_RESETS']
                err_msg = 'You are resetting passwords too frequently. Due to security policies, {} day(s) must elapse between password resets'.format(num_days)
    ...
    def _update_profile(self, request, existing_user):
        """
        Helper method to contain UserProfile update logic
        """
        existing_user_profile = UserProfile.objects.get(user_id=existing_user.id)
        if existing_user_profile:
            if existing_user.first_name and existing_user.last_name:
                existing_user_profile.name = '{} {}'.format(existing_user.first_name, existing_user.last_name)
            city = request.DATA.get('city')
            if city:
    ...
    def post(self, request, user_id):
        ...
        update_response = self._update_username(request, existing_user, response_data)
        if update_response is not None:
            return update_response
        update_response = self._update_password(request, existing_user, response_data)
        if update_response is not None:
            return update_response
        update_response = self._update_profile(request, existing_user)
        if update_response is not None:
            return update_response
        return Response(response_data, status=status.HTTP_200_OK)
    ...
    * Note: To create a new Course enrollment, see UsersCoursesList
    ...
    def get(self, request, user_id, course_id):
        ...
        try:
            ...
            course_descriptor, course_key, __ = get_course(request, user, course_id)
        except (ObjectDoesNotExist, ValueError):
            return Response({}, status=status.HTTP_404_NOT_FOUND)
        ...
        while parent_module is not None:
            ...
            if current_child_loc:
                ...
                __, __, parent_module = get_course_child(request, user, course_key, unicode(current_child_loc), load_content=True)
            else:
                parent_module = None
        return Response(response_data, status=status.HTTP_200_OK)

    def delete(self, request, user_id, course_id):  # pylint: disable=W0613
        """
