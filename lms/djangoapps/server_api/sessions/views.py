...
from server_api.users.serializers import UserSerializer
...
class SessionsDetail(SecureAPIView):
    ...
    **GET Response Values**
    ...
    def get(self, request, session_id):
        """
        GET /api/server/sessions/{session_id}
        """
        response_data = {}
        base_uri = generate_base_uri(request)
        engine = import_module(settings.SESSION_ENGINE)
    ...
    def delete(self, request, session_id):  # pylint: disable=W0613
        """
        DELETE /api/server/sessions/{session_id}
        """
        engine = import_module(settings.SESSION_ENGINE)
        session = engine.SessionStore(session_id)
