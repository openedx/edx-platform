...
def take_action_on_flags(request, course_id):
    ...
    for key in required:
        if key not in request.POST:
            error_message = _('Missing key {0} from submission.  Please reload and try again.').format(key)
            response = {
                'success': False,
    ...
    try:
        ...
        log.exception(
            _("Error taking action on flagged peer grading submissions, submission_id: {0}, action_type: {1}, grader_id: {2}").format(
