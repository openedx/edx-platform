...
class TestManagementUtils(unittest.TestCase):
    ...
    def test_user_not_found(self):
        ...
        self.assertRaises(User.DoesNotExist, management_utils.rename_user, '---@---.com', '---2@---2.com')

    def test_integrity_error(self):
        """
        ensures that an error is thrown when two users have the same username
        """
        self.assertRaises(IntegrityError, management_utils.rename_user, self.sql_users[0].username, self.sql_users[1].username)
    ...
    def test_rename_user(self):
        ...
        # perform the rename
        result = management_utils.rename_user(self.sql_users[0].username, TestManagementUtils.NEW_USERNAME)
