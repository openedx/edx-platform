...
class SkippedReverificationTest(ModuleStoreTestCase):
    ...
    def test_check_user_skipped_reverification_exists(self):
        ...
        self.assertTrue(
            SkippedReverification.check_user_skipped_reverification_exists(course_id=self.course.id, user=self.user.id)
        )

        user2 = UserFactory.create()
        self.assertFalse(
            SkippedReverification.check_user_skipped_reverification_exists(course_id=self.course.id, user=user2.id)
        )
