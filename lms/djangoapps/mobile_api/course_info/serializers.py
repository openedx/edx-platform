...
from lms.djangoapps.mobile_api.course_info.utils import (
    get_user_certificate_download_url,
)
...
class CourseInfoOverviewSerializer(serializers.ModelSerializer):
    ...
    def get_course_modes(self, course_overview):
        ...
        course_modes = CourseMode.modes_for_course(
            course_overview.id, only_selectable=False
        )
...
class CourseAccessSerializer(serializers.Serializer):
    ...
    has_unmet_prerequisites = serializers.SerializerMethodField(
        method_name="get_has_unmet_prerequisites"
    )
    ...
    def get_has_unmet_prerequisites(self, data: dict) -> bool:
        ...
        return any(
            get_pre_requisite_courses_not_completed(
                data.get("user"), [data.get("course_id")]
            )
        )
...
class CourseDetailSerializer(serializers.Serializer):
    ...
    def get_course_info_overview(self, data):
        ...
        return CourseInfoOverviewSerializer(
            course_overview, context=course_info_context
        ).data
    ...
    def get_discussion_url(data):
        ...
        return reverse(
            "discussion_course",
            kwargs={"course_id": data["course_id"]},
    ...
    def get_certificate(data):
        ...
        """
        return get_user_certificate_download_url(
            data["request"], data["user"], data["course_id"]
        )
    ...
    def get_enrollment_details(data):
        ...
        user_enrollment = CourseEnrollment.get_enrollment(
            user=data["user"], course_key=data["course_id"]
        )
    ...
    def get_course_handouts(data):
        ...
        return reverse(
            "course-handouts-list", kwargs=url_params, request=data["request"]
        )
    ...
    def get_course_updates(data):
        ...
        return reverse(
            "course-updates-list", kwargs=url_params, request=data["request"]
        )
