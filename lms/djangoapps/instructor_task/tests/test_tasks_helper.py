...
class TestGradeReport(TestReportMixin, InstructorTaskModuleTestCase):
    ...
    def create_course(self):
        ...
        """
        in_the_past = datetime.now(get_utc_timezone()) - timedelta(days=5)
        in_the_future = datetime.now(get_utc_timezone()) + timedelta(days=5)
        self.course = CourseFactory.create(
            grading_policy={
                "GRADER": [
                    {
...
class TestInstructorOra2Report(SharedModuleStoreTestCase):
    ...
    def test_report_stores_results(self):
        with ExitStack() as stack:
            ...
            timestamp_str = datetime.now(get_utc_timezone()).strftime('%Y-%m-%d-%H%M')
            key = self.course.id
