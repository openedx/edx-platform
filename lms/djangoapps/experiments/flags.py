...
class ExperimentWaffleFlag(CourseWaffleFlag):
    ...
    * the default bucket will be evaluated as if use_course_aware_bucketing=False.
    ...
    def _is_enrollment_inside_date_bounds(self, experiment_values, user, course_key):
        ...
        if enrollment_start:
            try:
                start_date = dateutil.parser.parse(enrollment_start).replace(tzinfo=get_utc_timezone())
            except ValueError:
        ...
        if enrollment_end:
            try:
                end_date = dateutil.parser.parse(enrollment_end).replace(tzinfo=get_utc_timezone())
            except ValueError:
