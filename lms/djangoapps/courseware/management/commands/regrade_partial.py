...
#
#   created < '2013-03-08 15:45:00' (the problem must have been answered before the fix was installed, on Prod and Edge)
...
class Command(BaseCommand):
    ...
    def fix_studentmodule_grade(self, module, save_changes):
        ...
        if module.grade == correct:
            # nothing to change
            log.debug("Grade matches for {type} module {id} for student {student} in course {course_id}".format(
                    type=module.module_type, id=module.module_state_key, student=module.student.username, course_id=module.course_id))
        elif save_changes:
            # make the change
