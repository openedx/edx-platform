...
def course_open_for_self_enrollment(course_key):
    ...
    now = datetime.now().replace(tzinfo=get_utc_timezone())
...
def date_block_key_fn(block):
    ...
    """
    return block.date or datetime.max.replace(tzinfo=get_utc_timezone())
...
def _ora_assessment_to_assignment(
    ...
    if date_config_type == 'subsection':
        ...
        if assessment.get('start'):
            assessment_start = parse_date(assessment.get('start')).replace(tzinfo=get_utc_timezone())
        if assessment.get('due'):
            assessment_due = parse_date(assessment.get('due')).replace(tzinfo=get_utc_timezone())
        extra_info = _(
