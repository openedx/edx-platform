...
from openedx.core.lib.time_zone_utils import get_utc_timezone  # lint-amnesty, pylint: disable=wrong-import-order
...
def can_show_verified_upgrade(user, enrollment, course=None):
    ...
    if datetime.datetime.now(get_utc_timezone()).date() > upgrade_deadline.date():
        return False
