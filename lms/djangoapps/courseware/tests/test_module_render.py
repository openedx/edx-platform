...
class ModuleRenderTestCase(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def setUp(self):
        ...
        self.callback_url = reverse('xqueue_callback', kwargs=dict(course_id=unicode(self.course_key),
    ...
    def test_module_render_with_jump_to_id(self):
        ...
        self.assertIn('/courses/' + unicode(self.course_key) + '/jump_to_id/vertical_test', html)
    ...
    def test_anonymous_handle_xblock_callback(self):
        dispatch_url = reverse(
            ...
            args=[
                ...
                quote_slashes(unicode(self.course_key.make_usage_key('videosequence', 'Toy_Videos'))),
                'xmodule_handler',
                'goto_position'
            ]
        )
    ...
    def test_too_many_files(self):
        ...
        self.assertEquals(
            render.handle_xblock_callback(
                request,
                unicode(self.course_key),
                quote_slashes(unicode(self.location)),
                'dummy_handler'
            ).content,
            json.dumps({
    ...
    def test_bad_location(self):
        ...
        with self.assertRaises(Http404):
            render.handle_xblock_callback(
                request,
                unicode(self.course_key),
                quote_slashes(unicode(self.course_key.make_usage_key('chapter', 'bad_location'))),
                'xmodule_handler',
                'goto_position',
            )
...
class TestModuleTrackingContext(ModuleStoreTestCase):
    ...
    def handle_callback_and_get_display_name_from_event(self, mock_tracker, problem_display_name=None):
        ...
        render.handle_xblock_callback(
            ...
            quote_slashes(unicode(descriptor.location)),
            'xmodule_handler',
            'problem_check',
        )
