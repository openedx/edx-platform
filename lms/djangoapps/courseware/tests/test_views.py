...
class IndexQueryTestCase(ModuleStoreTestCase):
    ...
    def test_index_query_counts(self):
        ...
        with self.assertNumQueries(187, table_ignorelist=QUERY_COUNT_TABLE_IGNORELIST):
...
class ProgressPageTests(ProgressPageBaseTests):
    ...
    def test_progress_queries(self):
        ...
        with self.assertNumQueries(
            61, table_ignorelist=QUERY_COUNT_TABLE_IGNORELIST
        ), check_mongo_calls(2):
            self._get_progress_page()

        for _ in range(2):
            with self.assertNumQueries(
                45, table_ignorelist=QUERY_COUNT_TABLE_IGNORELIST
            ), check_mongo_calls(2):
                self._get_progress_page()
