...
content_key = partial(
    LmsKeyValueStore.Key, Scope.content, None, location('def_id'))
settings_key = partial(
    LmsKeyValueStore.Key, Scope.settings, None, location('def_id'))
user_state_key = partial(
    LmsKeyValueStore.Key, Scope.user_state, 'user', location('def_id'))
...
class TestDescriptorFallback(TestCase):
    ...
    def test_write_to_descriptor(self):
        self.assertRaises(
            InvalidWriteError, self.kvs.set, content_key('field_a'), 'foo')
        self.assertEquals('content', self.desc_md['field_a'])
        self.assertRaises(
            InvalidWriteError, self.kvs.set, settings_key('field_b'), 'foo')
        self.assertEquals('settings', self.desc_md['field_b'])

        self.assertRaises(
            InvalidWriteError, self.kvs.delete, content_key('field_a'))
        self.assertEquals('content', self.desc_md['field_a'])
        self.assertRaises(
            InvalidWriteError, self.kvs.delete, settings_key('field_b'))
        self.assertEquals('settings', self.desc_md['field_b'])
...
class TestInvalidScopes(TestCase):
    def setUp(self):
        ...
        self.mdc = ModelDataCache([mock_descriptor([mock_field(
            Scope.user_state, 'a_field')])], course_id, self.user)
    ...
    def test_invalid_scopes(self):
        for scope in (Scope(user=True, block=BlockScope.DEFINITION),
            ...
            self.assertRaises(InvalidScopeError, self.kvs.get, LmsKeyValueStore.Key(
                scope, None, None, 'field'))
            self.assertRaises(InvalidScopeError, self.kvs.set, LmsKeyValueStore.Key(
                scope, None, None, 'field'), 'value')
            self.assertRaises(InvalidScopeError, self.kvs.delete, LmsKeyValueStore.Key(
                scope, None, None, 'field'))
            self.assertRaises(InvalidScopeError, self.kvs.has, LmsKeyValueStore.Key(
                scope, None, None, 'field'))
...
class TestStudentModuleStorage(TestCase):
    ...
    def test_delete_missing_field(self):
        ...
        self.assertRaises(
            KeyError, self.kvs.delete, user_state_key('not_a_field'))
        self.assertEquals(1, StudentModule.objects.all().count())
        self.assertEquals({'a_field': 'a_value'}, json.loads(
            StudentModule.objects.all()[0].state))
...
class TestMissingStudentModule(TestCase):
    ...
    def test_set_field_in_missing_student_module(self):
        ...
        self.assertEquals(location(
            'def_id').url(), student_module.module_state_key)
...
class StorageTestBase(object):
    ...
    def test_set_existing_field(self):
        ...
        self.assertEquals('new_value', json.loads(
    ...
    def test_set_missing_field(self):
        ...
        self.assertEquals('old_value', json.loads(
    ...
    def test_delete_missing_field(self):
        ...
        self.assertRaises(
            KeyError, self.kvs.delete, self.key_factory('missing_field'))
        self.assertEquals(1, self.storage_class.objects.all().count())
