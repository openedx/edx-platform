...
REQUIRE_BUILD_PROFILE = EDXAPP_CONFIG.get('REQUIRE_BUILD_PROFILE', REQUIRE_BUILD_PROFILE)
...
CC_MERCHANT_NAME = EDXAPP_CONFIG.get('CC_MERCHANT_NAME', PLATFORM_NAME)
EMAIL_FILE_PATH = EDXAPP_CONFIG.get('EMAIL_FILE_PATH', None)
EMAIL_HOST = EDXAPP_CONFIG.get('EMAIL_HOST', 'localhost')  # django default is localhost
EMAIL_PORT = EDXAPP_CONFIG.get('EMAIL_PORT', 25)  # django default is 25
EMAIL_USE_TLS = EDXAPP_CONFIG.get('EMAIL_USE_TLS', False)  # django default is False
...
DCS_SESSION_COOKIE_SAMESITE = EDXAPP_CONFIG.get('DCS_SESSION_COOKIE_SAMESITE', DCS_SESSION_COOKIE_SAMESITE)
DCS_SESSION_COOKIE_SAMESITE_FORCE_ALL = EDXAPP_CONFIG.get(
...
CMS_BASE = EDXAPP_CONFIG.get('CMS_BASE', 'studio.edx.org')

ALLOWED_HOSTS = [
    # TODO: bbeggs remove this before prod, temp fix to get load testing running
    "*",
    EDXAPP_CONFIG.get('LMS_BASE'),
...
# Currency
PAID_COURSE_REGISTRATION_CURRENCY = EDXAPP_CONFIG.get(
...
BULK_EMAIL_ROUTING_KEY = EDXAPP_CONFIG.get('BULK_EMAIL_ROUTING_KEY', HIGH_PRIORITY_QUEUE)

# We can run smaller jobs on the low priority queue. See note above for why
# we have to reset the value here.
BULK_EMAIL_ROUTING_KEY_SMALL_JOBS = EDXAPP_CONFIG.get('BULK_EMAIL_ROUTING_KEY_SMALL_JOBS', DEFAULT_PRIORITY_QUEUE)

# Queue to use for expiring old entitlements
ENTITLEMENTS_EXPIRATION_ROUTING_KEY = EDXAPP_CONFIG.get('ENTITLEMENTS_EXPIRATION_ROUTING_KEY', DEFAULT_PRIORITY_QUEUE)
...
if EDXAPP_CONFIG.get('COMPREHENSIVE_THEME_DIR', None):
    COMPREHENSIVE_THEME_DIR = EDXAPP_CONFIG.get('COMPREHENSIVE_THEME_DIR')
...
ENTERPRISE_MARKETING_FOOTER_QUERY_PARAMS = EDXAPP_CONFIG.get(
...
ID_VERIFICATION_SUPPORT_LINK = EDXAPP_CONFIG.get('ID_VERIFICATION_SUPPORT_LINK', SUPPORT_SITE_LINK)
PASSWORD_RESET_SUPPORT_LINK = EDXAPP_CONFIG.get('PASSWORD_RESET_SUPPORT_LINK', SUPPORT_SITE_LINK)
ACTIVATION_EMAIL_SUPPORT_LINK = EDXAPP_CONFIG.get('ACTIVATION_EMAIL_SUPPORT_LINK', SUPPORT_SITE_LINK)
LOGIN_ISSUE_SUPPORT_LINK = EDXAPP_CONFIG.get('LOGIN_ISSUE_SUPPORT_LINK', SUPPORT_SITE_LINK)
...
local_loglevel = EDXAPP_CONFIG.get('LOCAL_LOGLEVEL', 'INFO')
...
COURSE_LISTINGS = EDXAPP_CONFIG.get('COURSE_LISTINGS', {})
COMMENTS_SERVICE_URL = EDXAPP_CONFIG.get("COMMENTS_SERVICE_URL", '')
COMMENTS_SERVICE_KEY = EDXAPP_CONFIG.get("COMMENTS_SERVICE_KEY", '')
CERT_QUEUE = EDXAPP_CONFIG.get("CERT_QUEUE", 'test-pull')

# git repo loading  environment
GIT_REPO_DIR = EDXAPP_CONFIG.get('GIT_REPO_DIR', '/edx/var/edxapp/course_repos')
GIT_IMPORT_STATIC = EDXAPP_CONFIG.get('GIT_IMPORT_STATIC', True)
GIT_IMPORT_PYTHON_LIB = EDXAPP_CONFIG.get('GIT_IMPORT_PYTHON_LIB', True)
PYTHON_LIB_FILENAME = EDXAPP_CONFIG.get('PYTHON_LIB_FILENAME', 'python_lib.zip')
...
SSL_AUTH_EMAIL_DOMAIN = EDXAPP_CONFIG.get("SSL_AUTH_EMAIL_DOMAIN", "MIT.EDU")
SSL_AUTH_DN_FORMAT_STRING = EDXAPP_CONFIG.get(
...
CSRF_TRUSTED_ORIGINS = EDXAPP_CONFIG.get('CSRF_TRUSTED_ORIGINS', [])
...
if FEATURES.get('ENABLE_CORS_HEADERS') or FEATURES.get('ENABLE_CROSS_DOMAIN_CSRF_COOKIE'):
    ...
    CROSS_DOMAIN_CSRF_COOKIE_NAME = str(EDXAPP_CONFIG.get('CROSS_DOMAIN_CSRF_COOKIE_NAME'))
    ...
    CROSS_DOMAIN_CSRF_COOKIE_DOMAIN = EDXAPP_CONFIG.get('CROSS_DOMAIN_CSRF_COOKIE_DOMAIN')


# Field overrides. To use the IDDE feature, add
# 'courseware.student_field_overrides.IndividualStudentOverrideProvider'.
FIELD_OVERRIDE_PROVIDERS = tuple(EDXAPP_CONFIG.get('FIELD_OVERRIDE_PROVIDERS', []))
...
############### Module Store Items ##########
HOSTNAME_MODULESTORE_DEFAULT_MAPPINGS = EDXAPP_CONFIG.get('HOSTNAME_MODULESTORE_DEFAULT_MAPPINGS', {})
...
LMS_SEGMENT_KEY = EDXAPP_CONFIG.get('SEGMENT_KEY')
...
XQUEUE_INTERFACE = EDXAPP_CONFIG['XQUEUE_INTERFACE']

# Get the MODULESTORE from auth.json, but if it doesn't exist,
# use the one from common.py
MODULESTORE = convert_module_store_setting_if_needed(EDXAPP_CONFIG.get('MODULESTORE', MODULESTORE))
MONGODB_LOG = EDXAPP_CONFIG.get('MONGODB_LOG', {})

EMAIL_HOST_USER = EDXAPP_CONFIG.get('EMAIL_HOST_USER', '')  # django default is ''
EMAIL_HOST_PASSWORD = EDXAPP_CONFIG.get('EMAIL_HOST_PASSWORD', '')  # django default is ''
...
# Analytics API
ANALYTICS_API_KEY = EDXAPP_CONFIG.get("ANALYTICS_API_KEY", ANALYTICS_API_KEY)
ANALYTICS_API_URL = EDXAPP_CONFIG.get("ANALYTICS_API_URL", ANALYTICS_API_URL)

# Zendesk
ZENDESK_USER = EDXAPP_CONFIG.get("ZENDESK_USER")
ZENDESK_API_KEY = EDXAPP_CONFIG.get("ZENDESK_API_KEY")

# API Key for inbound requests from Notifier service
EDX_API_KEY = EDXAPP_CONFIG.get("EDX_API_KEY")
...
# upload limits
STUDENT_FILEUPLOAD_MAX_SIZE = EDXAPP_CONFIG.get("STUDENT_FILEUPLOAD_MAX_SIZE", STUDENT_FILEUPLOAD_MAX_SIZE)

# Event tracking
TRACKING_BACKENDS.update(EDXAPP_CONFIG.get("TRACKING_BACKENDS", {}))
EVENT_TRACKING_BACKENDS['tracking_logs']['OPTIONS']['backends'].update(EDXAPP_CONFIG.get("EVENT_TRACKING_BACKENDS", {}))
...
TRACKING_SEGMENTIO_WEBHOOK_SECRET = EDXAPP_CONFIG.get(
    "TRACKING_SEGMENTIO_WEBHOOK_SECRET",
    TRACKING_SEGMENTIO_WEBHOOK_SECRET
)
TRACKING_SEGMENTIO_ALLOWED_TYPES = EDXAPP_CONFIG.get(
    "TRACKING_SEGMENTIO_ALLOWED_TYPES",
    TRACKING_SEGMENTIO_ALLOWED_TYPES,
)
TRACKING_SEGMENTIO_DISALLOWED_SUBSTRING_NAMES = EDXAPP_CONFIG.get(
    "TRACKING_SEGMENTIO_DISALLOWED_SUBSTRING_NAMES",
    TRACKING_SEGMENTIO_DISALLOWED_SUBSTRING_NAMES
)
TRACKING_SEGMENTIO_SOURCE_MAP = EDXAPP_CONFIG.get("TRACKING_SEGMENTIO_SOURCE_MAP", TRACKING_SEGMENTIO_SOURCE_MAP)

# Heartbeat
HEARTBEAT_CELERY_ROUTING_KEY = EDXAPP_CONFIG.get('HEARTBEAT_CELERY_ROUTING_KEY', HEARTBEAT_CELERY_ROUTING_KEY)

# Student identity verification settings
VERIFY_STUDENT = EDXAPP_CONFIG.get("VERIFY_STUDENT", VERIFY_STUDENT)
DISABLE_ACCOUNT_ACTIVATION_REQUIREMENT_SWITCH = EDXAPP_CONFIG.get(
    "DISABLE_ACCOUNT_ACTIVATION_REQUIREMENT_SWITCH",
    DISABLE_ACCOUNT_ACTIVATION_REQUIREMENT_SWITCH
)

# Grades download
GRADES_DOWNLOAD_ROUTING_KEY = EDXAPP_CONFIG.get('GRADES_DOWNLOAD_ROUTING_KEY', HIGH_MEM_QUEUE)

GRADES_DOWNLOAD = EDXAPP_CONFIG.get("GRADES_DOWNLOAD", GRADES_DOWNLOAD)

# Rate limit for regrading tasks that a grading policy change can kick off

# financial reports
FINANCIAL_REPORTS = EDXAPP_CONFIG.get("FINANCIAL_REPORTS", FINANCIAL_REPORTS)
...
ORA2_FILE_PREFIX = EDXAPP_CONFIG.get("ORA2_FILE_PREFIX", ORA2_FILE_PREFIX)
...
LOGISTRATION_RATELIMIT_RATE = EDXAPP_CONFIG.get('LOGISTRATION_RATELIMIT_RATE', LOGISTRATION_RATELIMIT_RATE)
LOGISTRATION_API_RATELIMIT = EDXAPP_CONFIG.get('LOGISTRATION_API_RATELIMIT', LOGISTRATION_API_RATELIMIT)
...
TIME_ZONE_DISPLAYED_FOR_DEADLINES = EDXAPP_CONFIG.get(
...
if FEATURES.get('ENABLE_THIRD_PARTY_AUTH'):
    tmp_backends = EDXAPP_CONFIG.get('THIRD_PARTY_AUTH_BACKENDS', [
    ...
    SOCIAL_AUTH_PIPELINE_TIMEOUT = EDXAPP_CONFIG.get('SOCIAL_AUTH_PIPELINE_TIMEOUT', 600)

    # Most provider configuration is done via ConfigurationModels but for a few sensitive values
    # we allow configuration via EDXAPP_CONFIG instead (optionally).
    ...
    SOCIAL_AUTH_SAML_SP_PRIVATE_KEY = EDXAPP_CONFIG.get('SOCIAL_AUTH_SAML_SP_PRIVATE_KEY', '')
    SOCIAL_AUTH_SAML_SP_PUBLIC_CERT = EDXAPP_CONFIG.get('SOCIAL_AUTH_SAML_SP_PUBLIC_CERT', '')
    SOCIAL_AUTH_SAML_SP_PRIVATE_KEY_DICT = EDXAPP_CONFIG.get('SOCIAL_AUTH_SAML_SP_PRIVATE_KEY_DICT', {})
    SOCIAL_AUTH_SAML_SP_PUBLIC_CERT_DICT = EDXAPP_CONFIG.get('SOCIAL_AUTH_SAML_SP_PUBLIC_CERT_DICT', {})
    SOCIAL_AUTH_OAUTH_SECRETS = EDXAPP_CONFIG.get('SOCIAL_AUTH_OAUTH_SECRETS', {})
    SOCIAL_AUTH_LTI_CONSUMER_SECRETS = EDXAPP_CONFIG.get('SOCIAL_AUTH_LTI_CONSUMER_SECRETS', {})

    # third_party_auth config moved to ConfigurationModels. This is for data migration only:
    THIRD_PARTY_AUTH_OLD_CONFIG = EDXAPP_CONFIG.get('THIRD_PARTY_AUTH', None)

    if EDXAPP_CONFIG.get('THIRD_PARTY_AUTH_SAML_FETCH_PERIOD_HOURS', 24) is not None:
        CELERYBEAT_SCHEDULE['refresh-saml-metadata'] = {
            'task': 'common.djangoapps.third_party_auth.fetch_saml_metadata',
            'schedule': datetime.timedelta(hours=EDXAPP_CONFIG.get('THIRD_PARTY_AUTH_SAML_FETCH_PERIOD_HOURS', 24)),
        }
    ...
    THIRD_PARTY_AUTH_CUSTOM_AUTH_FORMS = EDXAPP_CONFIG.get('THIRD_PARTY_AUTH_CUSTOM_AUTH_FORMS', {})

##### OAUTH2 Provider ##############
if FEATURES.get('ENABLE_OAUTH2_PROVIDER'):
    OAUTH_ENFORCE_SECURE = EDXAPP_CONFIG.get('OAUTH_ENFORCE_SECURE', True)
    OAUTH_ENFORCE_CLIENT_SECURE = EDXAPP_CONFIG.get('OAUTH_ENFORCE_CLIENT_SECURE', True)
    # Defaults for the following are defined in lms.envs.common
    OAUTH_EXPIRE_DELTA = datetime.timedelta(
        days=EDXAPP_CONFIG.get('OAUTH_EXPIRE_CONFIDENTIAL_CLIENT_DAYS', OAUTH_EXPIRE_CONFIDENTIAL_CLIENT_DAYS)
    )
    OAUTH_EXPIRE_DELTA_PUBLIC = datetime.timedelta(
        days=EDXAPP_CONFIG.get('OAUTH_EXPIRE_PUBLIC_CLIENT_DAYS', OAUTH_EXPIRE_PUBLIC_CLIENT_DAYS)
    )


##### GOOGLE ANALYTICS IDS #####
GOOGLE_ANALYTICS_ACCOUNT = EDXAPP_CONFIG.get('GOOGLE_ANALYTICS_ACCOUNT')
GOOGLE_ANALYTICS_TRACKING_ID = EDXAPP_CONFIG.get('GOOGLE_ANALYTICS_TRACKING_ID')
GOOGLE_ANALYTICS_LINKEDIN = EDXAPP_CONFIG.get('GOOGLE_ANALYTICS_LINKEDIN')
GOOGLE_SITE_VERIFICATION_ID = EDXAPP_CONFIG.get('GOOGLE_SITE_VERIFICATION_ID')

##### BRANCH.IO KEY #####
BRANCH_IO_KEY = EDXAPP_CONFIG.get('BRANCH_IO_KEY')

#### Course Registration Code length ####
REGISTRATION_CODE_LENGTH = EDXAPP_CONFIG.get('REGISTRATION_CODE_LENGTH', 8)

# Which access.py permission names to check;
# We default this to the legacy permission 'see_exists'.
COURSE_CATALOG_VISIBILITY_PERMISSION = EDXAPP_CONFIG.get(
    'COURSE_CATALOG_VISIBILITY_PERMISSION',
    COURSE_CATALOG_VISIBILITY_PERMISSION
)
COURSE_ABOUT_VISIBILITY_PERMISSION = EDXAPP_CONFIG.get(
...
ENROLLMENT_COURSE_DETAILS_CACHE_TIMEOUT = EDXAPP_CONFIG.get('ENROLLMENT_COURSE_DETAILS_CACHE_TIMEOUT', 60)
...
if FEATURES.get('ENABLE_COURSEWARE_SEARCH') or \
   ...
   FEATURES.get('ENABLE_TEAMS'):
    ...
    SEARCH_FILTER_GENERATOR = EDXAPP_CONFIG.get('SEARCH_FILTER_GENERATOR', SEARCH_FILTER_GENERATOR)
...
# Facebook app
FACEBOOK_API_VERSION = EDXAPP_CONFIG.get("FACEBOOK_API_VERSION")
FACEBOOK_APP_SECRET = EDXAPP_CONFIG.get("FACEBOOK_APP_SECRET")
FACEBOOK_APP_ID = EDXAPP_CONFIG.get("FACEBOOK_APP_ID")
...
PROFILE_IMAGE_SIZES_MAP = EDXAPP_CONFIG.get(
...
CREDIT_PROVIDER_SECRET_KEYS = EDXAPP_CONFIG.get("CREDIT_PROVIDER_SECRET_KEYS", {})
...
LTI_USER_EMAIL_DOMAIN = EDXAPP_CONFIG.get('LTI_USER_EMAIL_DOMAIN', 'lti.example.com')

# For more info on this, see the notes in common.py
LTI_AGGREGATE_SCORE_PASSBACK_DELAY = EDXAPP_CONFIG.get(
...
STUDENTMODULEHISTORYEXTENDED_OFFSET = EDXAPP_CONFIG.get(
    'STUDENTMODULEHISTORYEXTENDED_OFFSET', STUDENTMODULEHISTORYEXTENDED_OFFSET
)

# Cutoff date for granting audit certificates
if EDXAPP_CONFIG.get('AUDIT_CERT_CUTOFF_DATE', None):
    AUDIT_CERT_CUTOFF_DATE = dateutil.parser.parse(EDXAPP_CONFIG.get('AUDIT_CERT_CUTOFF_DATE'))

################################ Settings for Credentials Service ################################

CREDENTIALS_GENERATION_ROUTING_KEY = EDXAPP_CONFIG.get('CREDENTIALS_GENERATION_ROUTING_KEY', DEFAULT_PRIORITY_QUEUE)

# Queue to use for award program certificates
PROGRAM_CERTIFICATES_ROUTING_KEY = EDXAPP_CONFIG.get('PROGRAM_CERTIFICATES_ROUTING_KEY', DEFAULT_PRIORITY_QUEUE)
SOFTWARE_SECURE_VERIFICATION_ROUTING_KEY = EDXAPP_CONFIG.get(
    'SOFTWARE_SECURE_VERIFICATION_ROUTING_KEY',
    HIGH_PRIORITY_QUEUE
)

API_ACCESS_MANAGER_EMAIL = EDXAPP_CONFIG.get('API_ACCESS_MANAGER_EMAIL')
API_ACCESS_FROM_EMAIL = EDXAPP_CONFIG.get('API_ACCESS_FROM_EMAIL')
...
ENTERPRISE_COURSE_ENROLLMENT_AUDIT_MODES = EDXAPP_CONFIG.get(
...
# message goes off.
ENTERPRISE_SUPPORT_URL = EDXAPP_CONFIG.get(
...
ENTERPRISE_API_URL = EDXAPP_CONFIG.get('ENTERPRISE_API_URL', DEFAULT_ENTERPRISE_API_URL)

DEFAULT_ENTERPRISE_CONSENT_API_URL = None
if LMS_INTERNAL_ROOT_URL is not None:
    DEFAULT_ENTERPRISE_CONSENT_API_URL = LMS_INTERNAL_ROOT_URL + '/consent/api/v1/'
ENTERPRISE_CONSENT_API_URL = EDXAPP_CONFIG.get('ENTERPRISE_CONSENT_API_URL', DEFAULT_ENTERPRISE_CONSENT_API_URL)

ENTERPRISE_SERVICE_WORKER_USERNAME = EDXAPP_CONFIG.get(
    'ENTERPRISE_SERVICE_WORKER_USERNAME',
    ENTERPRISE_SERVICE_WORKER_USERNAME
)
ENTERPRISE_API_CACHE_TIMEOUT = EDXAPP_CONFIG.get(
...
ENTERPRISE_PLATFORM_WELCOME_TEMPLATE = EDXAPP_CONFIG.get(
    'ENTERPRISE_PLATFORM_WELCOME_TEMPLATE',
    ENTERPRISE_PLATFORM_WELCOME_TEMPLATE
)
ENTERPRISE_SPECIFIC_BRANDED_WELCOME_TEMPLATE = EDXAPP_CONFIG.get(
    'ENTERPRISE_SPECIFIC_BRANDED_WELCOME_TEMPLATE',
    ENTERPRISE_SPECIFIC_BRANDED_WELCOME_TEMPLATE
)
ENTERPRISE_TAGLINE = EDXAPP_CONFIG.get(
    'ENTERPRISE_TAGLINE',
    ENTERPRISE_TAGLINE
)
...
BASE_COOKIE_DOMAIN = EDXAPP_CONFIG.get(
    'BASE_COOKIE_DOMAIN',
    BASE_COOKIE_DOMAIN
)
SYSTEM_TO_FEATURE_ROLE_MAPPING = EDXAPP_CONFIG.get(
    'SYSTEM_TO_FEATURE_ROLE_MAPPING',
    SYSTEM_TO_FEATURE_ROLE_MAPPING
)

# Add an ICP license for serving content in China if your organization is registered to do so
ICP_LICENSE = EDXAPP_CONFIG.get('ICP_LICENSE', None)
ICP_LICENSE_INFO = EDXAPP_CONFIG.get('ICP_LICENSE_INFO', {})
...
PARENTAL_CONSENT_AGE_LIMIT = EDXAPP_CONFIG.get(
...
MAINTENANCE_BANNER_TEXT = EDXAPP_CONFIG.get('MAINTENANCE_BANNER_TEXT', None)

########################## limiting dashboard courses ######################
DASHBOARD_COURSE_LIMIT = EDXAPP_CONFIG.get('DASHBOARD_COURSE_LIMIT', None)
