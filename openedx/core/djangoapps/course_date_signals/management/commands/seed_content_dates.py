...
class Command(BaseCommand):
    ...
    def _get_course_keys(self, options) -> List[CourseKey]:
        ...
        if options["course_id"]:
            try:
                ...
                raise CommandError(f"Invalid course ID format: {options['course_id']}")
    ...
    def _process_assignment_batch(self, course_key: CourseKey, assignments) -> tuple[int, int, int]:
        ...
        for assignment in assignments:
            self.stdout.write(f"Processing assignment: {assignment.block_key}/{assignment.assignment_type} (due: {assignment.date})")
