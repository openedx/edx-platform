...
from openedx.core.djangoapps.user_authn.views.utils import third_party_auth_context, third_party_auth_context_tpa_hint
...
class TPAContextDetailView(APIView):
    """
    API to get third party auth providers and the currently running pipeline.
    """
    throttle_classes = [ThirdPartyAuthContextThrottle]
    ...
    def get(self, request, **kwargs):  # lint-amnesty, pylint: disable=unused-argument
        ...
        if 'tpa_hint' not in request_params:
            return Response(
                status=status.HTTP_400_BAD_REQUEST,
                data={'message': 'Request missing required parameter: {}'.format(REDIRECT_KEY)}
            )
        ...
        context = third_party_auth_context_tpa_hint(request, redirect_to, third_party_auth_hint)
        return Response(
            status=status.HTTP_200_OK,
            data=context
        )
