...
class TestProgramProgressMeter(ModuleStoreTestCase):
    ...
    def test_single_program_multiple_entitlements(self, mock_get_programs):
        ...
        CourseEntitlementFactory.create(
            ...
            expired_at=datetime.datetime.now(get_utc_timezone()),
            mode=CourseMode.VERIFIED,
...
def _create_course(self, course_price, course_run_count=1, make_entitlement=False):
    ...
    for x in range(course_run_count):
        ...
        course.start = datetime.datetime.now(get_utc_timezone()) - datetime.timedelta(days=1)
        course.end = datetime.datetime.now(get_utc_timezone()) + datetime.timedelta(days=1)
...
class TestProgramDataExtender(ModuleStoreTestCase):
    ...
    def setUp(self):
        ...
        self.course.start = datetime.datetime.now(get_utc_timezone()) - datetime.timedelta(days=1)
        self.course.end = datetime.datetime.now(get_utc_timezone()) + datetime.timedelta(days=1)
    ...
    def test_is_enrollment_open(self, days_offset):
        ...
        """
        self.course.end = datetime.datetime.now(get_utc_timezone()) + datetime.timedelta(days=days_offset)
        self.course = self.update_course(self.course, self.user.id)
    ...
    def test_course_run_enrollment_status(self, start_offset, end_offset, is_enrollment_open):
        ...
        """
        self.course.enrollment_start = datetime.datetime.now(get_utc_timezone()) - datetime.timedelta(days=start_offset)
        self.course.enrollment_end = datetime.datetime.now(get_utc_timezone()) - datetime.timedelta(days=end_offset)
    ...
    def test_no_enrollment_start_date(self):
        ...
        """
        self.course.enrollment_end = datetime.datetime.now(get_utc_timezone()) - datetime.timedelta(days=1)
        self.course = self.update_course(self.course, self.user.id)
