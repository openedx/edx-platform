...
class TestProgramRetrieval(ProgramsApiConfigMixin, ProgramsDataMixin, TestCase):
    ...
    def test_get_programs_caching(self):
        ...
        Also verify that separate caches are used for staff and non-staff users.
        """
        self.create_config(cache_ttl=1)
        self.mock_programs_api()

        # Warm up the student cache.
        get_programs(self.user)

        # Hit the student cache.
        get_programs(self.user)
        ...
        # Warm up the staff cache.
        get_programs(staff_user)

        # Hit the staff cache.
        get_programs(staff_user)

        # Verify only two requests have been made (one for student, one for staff).
