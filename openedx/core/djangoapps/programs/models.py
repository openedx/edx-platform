...
class ProgramsApiConfig(ConfigurationModel):
    ...
    CACHE_KEY_ROOT = 'programs.api.data'
    ...
    def get_cache_key(self, user):
        """Construct the appropriate cache key for the given user."""
        return self.CACHE_KEY_ROOT + '.staff' if user.is_staff else self.CACHE_KEY_ROOT + '.student'
