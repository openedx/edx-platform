    ...
    def list(self, request):
        ...
        if usernames:
            ...
            if is_email_retired(user_email):
                ...
                retirement_id = None
                earliest_datetime = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=settings.COOL_OFF_DAYS)
                try:
                    retirement_status = UserRetirementStatus.objects.get(
                        created__gt=earliest_datetime,
                        created__lt=datetime.datetime.now(datetime.timezone.utc),
...
class AccountRetirementStatusView(ViewSet):
    ...
    def retirement_queue(self, request):
        ...
        try:
            ...
            earliest_datetime = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=cool_off_days)
    ...
    def retirements_by_status_and_date(self, request):
        ...
        """
        try:
            start_date = datetime.datetime.strptime(request.GET['start_date'], '%Y-%m-%d').replace(tzinfo=datetime.timezone.utc)
            end_date = datetime.datetime.strptime(request.GET['end_date'], '%Y-%m-%d').replace(tzinfo=datetime.timezone.utc)
