    ...
    def test_register_duplicate_username_account_validation_error(self, dummy_check_account_exists):
        ...
        self.assertEqual(
            response_json,
            {
                "username": [{
                    "user_message": (
                        "An account with the Public Username '{}' already exists."
                    ).format(
                        self.USERNAME
                    )
                }]
...
class RegistrationViewTest(ThirdPartyAuthTestMixin, UserAPITestCase):
    ...
    def test_registration_honor_code_mktg_site_enabled(self):
        ...
        self._assert_reg_field(
            {"honor_code": "required"},
            {
                "label": ("By creating an account with {platform_name}, you agree {spacing}"
                          "to abide by our {platform_name} {spacing}"
                          "{link_label} {spacing}"
                          "and agree to our {link_label2}.").format(
                    platform_name=settings.PLATFORM_NAME,
    ...
    def _assert_fields_match(self, actual_field, expected_field):
        ...
        for key, value in expected_field.items():
            self.assertEqual(
                ...
                msg="Expected {expected} for {key} but got {actual} instead".format(
                    key=key,
...
class UpdateEmailOptInTestCase(UserAPITestCase, SharedModuleStoreTestCase):
    ...
    def test_update_email_opt_in_inactive_user(self):
        ...
        self.assertEqual(preference.value, "True")
