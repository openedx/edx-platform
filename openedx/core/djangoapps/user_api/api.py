...
class RegistrationFormFactory(object):
    ...
    def get_registration_form(self, request):
        ...
        if custom_form:
            ...
            for field_name, field in list(custom_form.fields.items()):
                restrictions = {}
    ...
    def _add_field_with_configurable_select_options(self, field_name, field_label, form_desc, required=False):
        ...
        if extra_field_options is None or extra_field_options.get(field_name) is None:
            ...
            options = None
            error_msg = ''
            error_msg = getattr(accounts, 'REQUIRED_FIELD_{}_TEXT_MSG'.format(field_name.upper()))
        else:
            field_type = "select"
            include_default_option = True
            ...
            error_msg = ''
            error_msg = getattr(accounts, 'REQUIRED_FIELD_{}_SELECT_MSG'.format(field_name.upper()))
    ...
    def _add_honor_code_field(self, form_desc, required=True):
        ...
        if not separate_honor_and_tos:
            ...
            label = Text(_(
                "By creating an account with {platform_name}, you agree \
