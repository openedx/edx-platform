...
class IsCourseStaffInstructor(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        return (hasattr(request, 'user') and
                ...
                # user_has_permission_list_course below will never return true.
...
class IsStaffOrReadOnly(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        return (request.user.is_staff or
                CourseStaffRole(obj.course_id).has_user(request.user) or
                user_has_permission_course(request.user, CourseRolesPermission.MANAGE_STUDENTS.value, obj.course_id) or
                request.method in permissions.SAFE_METHODS)
