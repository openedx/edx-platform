...
from .helpers import populate_trophycase, get_course_badges, add_posts_data_in_badges_list,\
    get_discussion_team_ids, get_badge_progress_request_data
from .models import UserBadge
from .constants import BADGES_KEY, COURSES_KEY, COURSE_ID_KEY, COMMUNITY_URL_KEY
...
def trophycase(request):
    ...
    enrolled_courses_data = CourseEnrollment.enrollments_for_user(user).order_by(
        'course__display_name').values_list(COURSE_ID_KEY, 'course__display_name')
    ...
    course_key_list = [CourseKey.from_string(unicode(course)) for course in trophycase_dict.keys()]
    ...
    if discussion_courses:
        ...
        ## WE are passing 0 as discussion id in case DiscussionCommunity didn't provide us with any data so that our
        ## API won't break.
    ...
    if response:
        for course in response[COURSES_KEY]:
            add_posts_data_in_badges_list(course, course.keys()[0], trophycase_dict[course.keys()[0]][BADGES_KEY])
...
def my_badges(request, course_id):
    ...
    ## Here we are dealing with just one course so we can wrap this course details in '[]' because our API on NOdeBB
    ## side only accepts params in this format.
    ...
    if response:
        ...
        add_posts_data_in_badges_list(response[COURSES_KEY][0], course_id, badges[BADGES_KEY])
