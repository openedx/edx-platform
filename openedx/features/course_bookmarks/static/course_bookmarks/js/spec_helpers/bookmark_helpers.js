...
function BookmarkHelpers() {
  ...
  const TEST_COURSE_ID = 'course-v1:test-course';

  const createBookmarksData = (options) => {
    const data = {
    ...
    for (i = 0; i < options.numBookmarksToCreate; i += 1) {
      bookmarkInfo = {
        id: i,
  ...
  const createBookmarkUrl = (courseId, usageId) => `/courses/${courseId}/jump_to/${usageId}`;

  const breadcrumbTrail = (path, unitDisplayName) => _.pluck(path, 'display_name')
    .concat([unitDisplayName])
    .join(' <span class="icon fa fa-caret-right" aria-hidden="true"></span><span class="sr">-</span> ');

  const verifyBookmarkedData = (view, expectedData) => {
    let courseId;
    let usageId;
    const bookmarks = view.$('.bookmarks-results-list-item');
    const results = expectedData.results;
    let i;
    let $bookmark;
  ...
  const verifyPaginationInfo = (requests, view, expectedData, currentPage, headerMessage) => {
