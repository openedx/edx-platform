...
def test_next_course_short_id():
    """
    Assert that course short id's are generating in a sequence starting from 100
    """
    ...
    assert course_meta_list[-1].course_short_id == 111
...
def test_next_course_short_id_operational_error(mocker):
    ...
    `course_meta` table do not have `course_short_id` field during migration)
    """
