...
def test_command_update_is_bu_prerequisite_courses_no_users_to_be_checked_for_update(mocker, prereq_course_groups):
    """
...
@pytest.mark.django_db
def test_update_is_bu_prerequisite_courses_passed_in_application_hub(caplog):
    """
    Assert that `is_bu_prerequisite_courses_passed` flag is successfully updated in application hub model
    """
    ...
    command = command_module.Command()
    command.update_is_bu_prerequisite_courses_passed_in_application_hub(users)
    ...
    assert '`is_bu_prerequisite_courses_passed` flag is updated' in caplog.messages[0]
...
def test_get_users_with_bu_pre_reqs_not_marked_as_passed(
    ...
    """
    Assert that users are only returned if application is completed and program prereqs are passed
    ...
    command = command_module.Command()
    filtered_users = command.get_users_with_bu_pre_reqs_not_marked_as_passed()
