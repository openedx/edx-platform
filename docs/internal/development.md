...
1. Each content directory in edx/data should be a clone of a git repo
...
4. The git post-receive-hook should POST to /gitreload with a JSON payload.  This payload should define at least
   ...
   { "repository" : { "name" : reload_dir }
    ...
    where reload_dir is the directory name of the content to reload (ie edx/data/reload_dir should exist)
