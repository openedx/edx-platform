...
27, 2014. See previous pages in this document for a history of changes.
...
* This release further expands the recently enhanced Studio workflow for
  ...
  * You can now publish all unpublished content in an entire section or
    ...
    The "Publish" icon is also available for 
    individual units on the **Course Outline** page.
...
* When you click **Sent Email History** on the Instructor Dashboard **Email**
  page, the list of messages now identifies the sender of each message in a new
  **Sent By** column.  
  
* The Instructor Dashboard no longer displays instructions on how to access an
  older version. The **Revert to Legacy Dashboard** button is still available
  to provide access if needed.
...
* The computations for demographic distributions that are reported on the
  **Analytics** page of the Instructor Dashboard have been updated. The level
  of education and gender distributions now include currently enrolled students
  only. Previously, these distributions were computed for all students who were
  ever enrolled in the course. (LMS-11228)
...
We're pleased to announce these changes, which will be released in the
next few weeks:
...
All guides now include a section that lists browsers supported for
use with the edX platform.
...
* `Building and Running an edX Course`_ 
  ...
  * The chapter on "Developing Your Course" has been updated to include the new
    capabilities for publishing and hiding multiple units on the **Course
    Outline** page.

  * A new chapter on "Creating Content Experiments" is now available.

  * An additional sample email message for students, announcing a new run of a
    course, is now available in the "Course Launching Activities" section.

  * Messages about the effect of changes to the grading policy, graded
    subsections, or graded components in a live course are now included.
...
* `edX Research Guide`_
   ...
   * The "Tracking Logs" chapter now includes a section on open response
     assessment events.

   * Chapters on "Course Grading" and "XML format of drag and drop input" were
